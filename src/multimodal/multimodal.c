...
void media_config_init(GV_MediaConfig *config) {
    if (!config) return;
...
static int compute_sha256_hex(const void *data, size_t len, char *hex_out) {
    ...
    if (auth_sha256(data, len, raw) != 0) return -1;
    auth_to_hex(raw, 32, hex_out);
    return 0;
}
...
void media_destroy(GV_MediaStore *store) {
    if (!store) return;
...
int media_store_blob(GV_MediaStore *store, size_t vector_index,
    ...
    node->entry.filename = filename ? gv_dup_cstr(filename) : NULL;
    node->entry.file_size = data_size;
    memcpy(node->entry.hash, hash_hex, 65);
    node->entry.created_at = (uint64_t)time(NULL);
    node->entry.mime_type = mime_type ? gv_dup_cstr(mime_type) : NULL;
...
int media_store_file(GV_MediaStore *store, size_t vector_index,
...
int media_retrieve(const GV_MediaStore *store, size_t vector_index,
...
int media_get_path(const GV_MediaStore *store, size_t vector_index,
                       char *path, size_t path_size) {
...
int media_get_info(const GV_MediaStore *store, size_t vector_index,
                       GV_MediaEntry *entry) {
    ...
    entry->mime_type = node->entry.mime_type ? gv_dup_cstr(node->entry.mime_type) : NULL;

    pthread_rwlock_unlock((pthread_rwlock_t *)&store->lock);
    return 0;
}

int media_delete(GV_MediaStore *store, size_t vector_index) {
    if (!store) return -1;
...
size_t media_count(const GV_MediaStore *store) {
    if (!store) return 0;
...
size_t media_total_size(const GV_MediaStore *store) {
    if (!store) return 0;
...
GV_MediaStore *media_load_index(const char *index_path,
