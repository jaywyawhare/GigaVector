...
void tokenizer_config_init(GV_TokenizerConfig *config) {
    if (!config) return;
...
int tokenizer_tokenize(GV_Tokenizer *tokenizer, const char *text,
...
void token_list_free(GV_TokenList *list) {
    if (!list) return;
...
int tokenize_simple(const char *text, GV_TokenList *result) {
    GV_Tokenizer *tokenizer = tokenizer_create(NULL);
    if (!tokenizer) return -1;

    int ret = tokenizer_tokenize(tokenizer, text, 0, result);
    tokenizer_destroy(tokenizer);

    return ret;
}

int token_list_unique(const GV_TokenList *list, char ***unique_tokens, size_t *unique_count) {
    ...
    for (size_t i = 0; i < list->count; i++) {
        ...
        if (!found) {
            tokens[count] = gv_dup_cstr(list->tokens[i].text);
            if (!tokens[count]) {
                unique_tokens_free(tokens, count);
                return -1;
            }
            count++;
        }
