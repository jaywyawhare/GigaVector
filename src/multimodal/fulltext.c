...
int ft_stem(const char *word, GV_FTLanguage lang, char *output, size_t output_size) {
...
static int ft_tokenize(const char *text, GV_FTLanguage lang,
    ...
    while (i < tlen) {
        ...
        if (do_stem) {
            ...
            if (ft_stem(word, lang, stemmed, sizeof(stemmed)) == 0) {
...
void ft_destroy(GV_FTIndex *idx) {
    if (!idx) return;
...
int ft_remove_document(GV_FTIndex *idx, size_t doc_id) {
    if (!idx) return -1;
...
int ft_search(const GV_FTIndex *idx, const char *query, size_t limit,
                 GV_FTResult *results) {
...
int ft_search_phrase(const GV_FTIndex *idx, const char *phrase, size_t limit,
                        GV_FTResult *results) {
...
size_t ft_doc_count(const GV_FTIndex *idx) {
    if (!idx) return 0;
    pthread_rwlock_rdlock((pthread_rwlock_t *)&idx->rwlock);
...
int ft_save(const GV_FTIndex *idx, const char *path) {
...
GV_FTIndex *ft_load(const char *path) {
    if (!path) return NULL;
