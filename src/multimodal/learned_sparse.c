 ...
 */
static int ls_posting_append(GV_LSPostingList *pl, size_t doc_id, float weight,
                                size_t block_size) {
...
GV_MIN_HEAP_DEFINE(ls_heap, GV_LSHeapItem)
...
static void ls_score_map_destroy(GV_LSScoreMap *map) {
    if (!map) return;
 ...
 */
static size_t ls_cursor_doc(const GV_LSWandCursor *c) {
 ...
 */
static void ls_cursor_advance_to(GV_LSWandCursor *c, size_t target,
                                     size_t block_size) {
...
static int ls_search_wand(const GV_LearnedSparseIndex *idx,
    ...
    for (size_t i = 0; i < query_count; i++) {
        ...
        float global_max = ls_posting_list_max_weight(pl);
    ...
    while (1) {
        ...
        size_t pivot_doc = ls_cursor_doc(&cursors[pivot]);
        if (pivot_doc == SIZE_MAX) break;

        size_t first_doc = ls_cursor_doc(&cursors[0]);

        if (first_doc == pivot_doc) {
            if (!idx->docs[pivot_doc].deleted) {
                float score = 0.0f;
                for (size_t c = 0; c < num_cursors; c++) {
                    if (ls_cursor_doc(&cursors[c]) != pivot_doc) break;
                    score += cursors[c].query_weight *
                             cursors[c].pl->postings[cursors[c].cursor].weight;
                }

                if (score >= min_score) {
                    ls_heap_push(heap, &heap_size, k, (GV_LSHeapItem){score, pivot_doc});
                }
            }
            ...
            for (size_t c = 0; c < pivot; c++) {
                if (ls_cursor_doc(&cursors[c]) < pivot_doc) {
                    ls_cursor_advance_to(&cursors[c], pivot_doc, block_size);
                }
            }
        }
    }
    ...
    for (int i = n - 1; i >= 0; i--) {
        ...
        heap_size--;
        if (heap_size > 0) {
            ls_heap_sift_down(heap, heap_size, 0);
        }
    }
...
static int ls_search_accumulate(const GV_LearnedSparseIndex *idx,
                                    const GV_LSSparseEntry *query,
                                    size_t query_count,
                                    float min_score, size_t k,
                                    GV_LearnedSparseResult *results) {
    GV_LSScoreMap *map = ls_score_map_create(GV_LS_SCORE_MAP_BUCKETS);
    if (!map) return -1;
    ...
    for (size_t q = 0; q < query_count; q++) {
        ...
        for (size_t p = 0; p < pl->count; p++) {
            ...
            float *score_ptr = ls_score_map_get_or_insert(map, doc_id);
            if (!score_ptr) {
    ...
    for (size_t b = 0; b < map->num_buckets; b++) {
        ...
        while (e) {
            if (e->used && e->score >= min_score) {
                ls_heap_push(heap, &heap_size, k, (GV_LSHeapItem){e->score, e->doc_id});
            }
            e = e->next;
        }
    }
...
int ls_insert(GV_LearnedSparseIndex *idx, const GV_LSSparseEntry *entries,
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        if (ls_posting_append(&idx->posting_lists[tid], doc_id,
...
int ls_search(const GV_LearnedSparseIndex *idx, const GV_LSSparseEntry *query,
    ...
    if (idx->config.use_wand) {
        n = ls_search_wand(idx, query, query_count, 0.0f, k, results);
    } else {
        n = ls_search_accumulate(idx, query, query_count, 0.0f, k, results);
    }
...
int ls_search_with_threshold(const GV_LearnedSparseIndex *idx,
    ...
    int n;
    if (idx->config.use_wand) {
        n = ls_search_wand(idx, query, query_count, min_score, k, results);
    } else {
        n = ls_search_accumulate(idx, query, query_count, min_score, k,
                                     results);
    }
...
int ls_get_stats(const GV_LearnedSparseIndex *idx,
...
int ls_save(const GV_LearnedSparseIndex *idx, const char *path) {
    ...
    if (write_u32(fp, GV_LS_VERSION) != 0) goto fail;

    if (write_u64(fp, (uint64_t)idx->config.vocab_size) != 0) goto fail;
    if (write_u64(fp, (uint64_t)idx->config.max_nonzeros) != 0) goto fail;
    if (write_u32(fp, (uint32_t)idx->config.use_wand) != 0) goto fail;
    if (write_u64(fp, (uint64_t)idx->config.wand_block_size) != 0) goto fail;

    if (write_u64(fp, (uint64_t)idx->doc_count) != 0) goto fail;

    for (size_t i = 0; i < idx->doc_count; i++) {
        if (write_u64(fp, (uint64_t)idx->docs[i].entry_count) != 0) goto fail;
        if (write_u32(fp, (uint32_t)idx->docs[i].deleted) != 0) goto fail;
    }
    ...
    if (write_u64(fp, non_empty_count) != 0) goto fail;

    for (size_t i = 0; i < idx->config.vocab_size; i++) {
        const GV_LSPostingList *pl = &idx->posting_lists[i];
        if (pl->count == 0) continue;

        if (write_u32(fp, (uint32_t)i) != 0) goto fail;
        if (write_u64(fp, (uint64_t)pl->count) != 0) goto fail;

        for (size_t j = 0; j < pl->count; j++) {
            if (write_u64(fp, (uint64_t)pl->postings[j].doc_id) != 0) goto fail;
            if (write_f32(fp, pl->postings[j].weight) != 0) goto fail;
        }
    }
...
GV_LearnedSparseIndex *ls_load(const char *path) {
    ...
    uint32_t version = 0;
    if (read_u32(fp, &version) != 0 || version != GV_LS_VERSION) {
        fclose(fp);
        return NULL;
    }
    ...
    uint32_t use_wand = 0;
    if (read_u64(fp, &vocab_size) != 0)      { fclose(fp); return NULL; }
    if (read_u64(fp, &max_nonzeros) != 0)    { fclose(fp); return NULL; }
    if (read_u32(fp, &use_wand) != 0)        { fclose(fp); return NULL; }
    if (read_u64(fp, &wand_block_size) != 0) { fclose(fp); return NULL; }
    ...
    if (read_u64(fp, &doc_count_raw) != 0) { fclose(fp); return NULL; }

    GV_LearnedSparseIndex *idx = ls_create(&cfg);
    ...
    for (size_t i = 0; i < doc_count; i++) {
        uint64_t ec = 0;
        uint32_t del = 0;
        if (read_u64(fp, &ec) != 0 || read_u32(fp, &del) != 0) {
            ls_destroy(idx);
            fclose(fp);
            return NULL;
        }
    ...
    uint64_t non_empty_count = 0;
    if (read_u64(fp, &non_empty_count) != 0) {
        ls_destroy(idx);
        fclose(fp);
        return NULL;
    }
    ...
    for (uint64_t t = 0; t < non_empty_count; t++) {
        uint32_t tid = 0;
        uint64_t pcount = 0;
        if (read_u32(fp, &tid) != 0 || read_u64(fp, &pcount) != 0) {
            ls_destroy(idx);
            fclose(fp);
            return NULL;
        }
        ...
        for (uint64_t p = 0; p < pcount; p++) {
            uint64_t did = 0;
            float    w   = 0.0f;
            if (read_u64(fp, &did) != 0 || read_f32(fp, &w) != 0) {
                ls_destroy(idx);
                fclose(fp);
                return NULL;
            }
