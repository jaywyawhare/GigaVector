...
void bm25_config_init(GV_BM25Config *config) {
    if (!config) return;
...
void bm25_destroy(GV_BM25Index *index) {
    if (!index) return;
...
static GV_PostingList *find_posting_list(GV_BM25Index *index, const char *term) {
    size_t bucket = hash_str(term) % TERM_HASH_BUCKETS;
...
int bm25_add_document(GV_BM25Index *index, size_t doc_id, const char *text) {
    ...
    char **unique_terms;
    size_t unique_count;
    if (token_list_unique(&tokens, &unique_terms, &unique_count) != 0) {
    ...
    if (!di) {
        ...
        unique_tokens_free(unique_terms, unique_count);
        token_list_free(&tokens);
        return -1;
    }
...
int bm25_remove_document(GV_BM25Index *index, size_t doc_id) {
    if (!index) return -1;
...
int bm25_update_document(GV_BM25Index *index, size_t doc_id, const char *text) {
    bm25_remove_document(index, doc_id);
...
int bm25_search(GV_BM25Index *index, const char *query, size_t k,
                   GV_BM25Result *results) {
    if (!index || !query || !results || k == 0) return -1;

    GV_TokenList tokens;
    if (tokenizer_tokenize(index->tokenizer, query, 0, &tokens) != 0) {
        return -1;
    }
    ...
    int result = bm25_search_terms(index, (const char **)unique_terms, unique_count,
                                       k, results);
...
int bm25_search_terms(GV_BM25Index *index, const char **terms, size_t term_count,
...
int bm25_score_document(GV_BM25Index *index, size_t doc_id, const char *query,
                            double *score) {
...
int bm25_get_stats(const GV_BM25Index *index, GV_BM25Stats *stats) {
...
size_t bm25_get_doc_freq(const GV_BM25Index *index, const char *term) {
...
int bm25_has_document(const GV_BM25Index *index, size_t doc_id) {
    if (!index) return 0;
...
int bm25_save(const GV_BM25Index *index, const char *filepath) {
...
GV_BM25Index *bm25_load(const char *filepath) {
