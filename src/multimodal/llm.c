...
static int parse_openai_response(const char *response_json, GV_LLMResponse *out) {
    ...
    const char *content = json_get_string_path(root, "choices.0.message.content");
    if (content == NULL) {
        json_free(root);
        return -1;
    }

    out->content = gv_dup_cstr(content);
    if (out->content == NULL) {
        json_free(root);
        return -1;
    }
    ...
    if (finish && json_is_string(finish)) {
        const char *reason = json_get_string(finish);
...
static int parse_gemini_response(const char *response_json, GV_LLMResponse *out) {
    ...
    const char *content = json_get_string_path(root, "candidates.0.content.parts.0.text");
    if (content == NULL) {
        json_free(root);
        return -1;
    }
...
static int parse_anthropic_response(const char *response_json, GV_LLMResponse *out) {
    ...
    GV_JsonValue *content_array = json_object_get(root, "content");
    if (content_array == NULL || !json_is_array(content_array)) {
        json_free(root);
        return -1;
    }
    ...
    size_t array_len = json_array_length(content_array);
    for (size_t i = 0; i < array_len; i++) {
        GV_JsonValue *block = json_array_get(content_array, i);
        if (block && json_is_object(block)) {
            GV_JsonValue *type_val = json_object_get(block, "type");
            if (type_val && json_is_string(type_val)) {
                const char *type = json_get_string(type_val);
                if (type && strcmp(type, "text") == 0) {
                    text_content = json_get_string(json_object_get(block, "text"));
                    break;
                }
            }
        }
    ...
    out->content = gv_dup_cstr(text_content);
    if (out->content == NULL) {
        json_free(root);
        return -1;
    }
    ...
    if (stop_reason && json_is_string(stop_reason)) {
        const char *reason = json_get_string(stop_reason);
...
GV_LLM *llm_create(const GV_LLMConfig *config) {
    if (config == NULL) {
        return NULL;
    }
...
const char *llm_get_last_error(GV_LLM *llm) {
    if (llm == NULL) {
        return NULL;
    }
...
void llm_response_free(GV_LLMResponse *response) {
    if (response == NULL) {
        return;
    }
...
void llm_message_free(GV_LLMMessage *message) {
    if (message == NULL) {
        return;
    }
...
void llm_messages_free(GV_LLMMessage *messages, size_t count) {
    if (messages == NULL) {
        return;
    }
