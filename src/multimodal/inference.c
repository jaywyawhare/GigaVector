 ...
 *
 * Wraps the auto-embedding API (auto_embed.h) and the database API
 * (database.h) to provide a single-call interface: supply plain text,
...
static char *inf_extract_text(const GV_Metadata *meta) {
    while (meta) {
        if (meta->key && strcmp(meta->key, INF_META_KEY_TEXT) == 0) {
            return meta->value ? gv_dup_cstr(meta->value) : NULL;
        }
        meta = meta->next;
    }
    return NULL;
...
static size_t inf_resolve_index(const GV_Database *db,
    ...
    for (size_t i = 0; i < count; i++) {
        const float *candidate = database_get_vector(db, i);
...
static int inf_populate_result(const GV_Database *db,
    ...
    if (out->index != (size_t)-1 && db->soa_storage) {
        GV_Metadata *meta = soa_storage_get_metadata(db->soa_storage,
                                                         out->index);
        if (meta) {
...
GV_InferenceEngine *inference_create(void *db,
...
void inference_destroy(GV_InferenceEngine *eng) {
    if (!eng) return;
...
int inference_add_batch(GV_InferenceEngine *eng, const char **texts,
     ...
     * auto_embed_add_texts for a true batch HTTP call, but that API
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        size_t dim = 0;
        float *vec = auto_embed_text(eng->embedder, texts[i], &dim);
...
int inference_upsert(GV_InferenceEngine *eng, size_t index,
    ...
    int rc = db_upsert_with_metadata(
