...
static void ae_cache_evict_lru(AE_Cache *c) {
    ...
    size_t h = hash_str(victim->text) % c->bucket_count;
...
static int ae_cache_get(AE_Cache *c, const char *text,
    ...
    size_t h = hash_str(text) % c->bucket_count;
...
static int ae_parse_openai_single(const char *body, float **out_vec,
                                  size_t *out_dim) {
    GV_JsonError err;
    GV_JsonValue *root = json_parse(body, &err);
    if (!root) return -1;

    GV_JsonValue *embedding_arr = json_get_path(root, "data.0.embedding");
    if (!embedding_arr || !json_is_array(embedding_arr)) {
        json_free(root);
        return -1;
    }

    size_t dim = json_array_length(embedding_arr);
    if (dim == 0) {
        json_free(root);
        return -1;
    }
    ...
    for (size_t i = 0; i < dim; i++) {
        GV_JsonValue *elem = json_array_get(embedding_arr, i);
        double val = 0.0;
...
static int ae_parse_openai_batch(const char *body, size_t count,
    ...
    GV_JsonValue *data_arr = json_object_get(root, "data");
    if (!data_arr || !json_is_array(data_arr)) {
        json_free(root);
        return -1;
    }

    size_t arr_len = json_array_length(data_arr);
    if (arr_len < count) count = arr_len;

    int success = 0;
    for (size_t i = 0; i < count; i++) {
        GV_JsonValue *item = json_array_get(data_arr, i);
        if (!item) { out_vecs[i] = NULL; out_dims[i] = 0; continue; }

        GV_JsonValue *emb = json_object_get(item, "embedding");
        if (!emb || !json_is_array(emb)) {
        ...
        for (size_t j = 0; j < dim; j++) {
            GV_JsonValue *e = json_array_get(emb, j);
            double v = 0.0;
            if (e) json_get_number(e, &v);
            vec[j] = (float)v;
        }
        out_vecs[i] = vec;
        out_dims[i] = dim;
...
static int ae_parse_google_single(const char *body, float **out_vec,
    ...
    GV_JsonValue *values = json_get_path(root, "embedding.values");
    ...
    for (size_t i = 0; i < dim; i++) {
        GV_JsonValue *e = json_array_get(values, i);
        double v = 0.0;
        if (e) json_get_number(e, &v);
        vec[i] = (float)v;
    }
...
static int ae_parse_google_batch(const char *body, size_t count,
    ...
    GV_JsonValue *embeddings = json_object_get(root, "embeddings");
    if (!embeddings || !json_is_array(embeddings)) {
        json_free(root);
        return -1;
    }

    size_t arr_len = json_array_length(embeddings);
    if (arr_len < count) count = arr_len;

    int success = 0;
    for (size_t i = 0; i < count; i++) {
        GV_JsonValue *item = json_array_get(embeddings, i);
        GV_JsonValue *vals = item ? json_object_get(item, "values") : NULL;
        if (!vals || !json_is_array(vals)) {
            out_vecs[i] = NULL; out_dims[i] = 0; continue;
        }
        ...
        for (size_t j = 0; j < dim; j++) {
            GV_JsonValue *e = json_array_get(vals, j);
            double v = 0.0;
            if (e) json_get_number(e, &v);
            vec[j] = (float)v;
        }
...
int auto_embed_add_text(GV_AutoEmbedder *embedder, GV_Database *db,
    ...
    if (metadata_key && metadata_value) {
        rc = db_add_vector_with_metadata(db, vec, dim, metadata_key, metadata_value);
    } else {
...
int auto_embed_search_text(GV_AutoEmbedder *embedder, const GV_Database *db,
     ...
     */
    for (size_t i = 0; i < n; i++) {
        size_t db_count = database_count(db);
        size_t idx = 0;
        for (size_t j = 0; j < db_count; j++) {
            const float *candidate = database_get_vector(db, j);
    ...
    for (size_t offset = 0; offset < count; offset += batch_sz) {
        ...
        if (need_count > 0) {
            ...
            if (batch_ok < 0) {
                for (size_t i = 0; i < need_count; i++) {
                    ...
                    vecs[idx] = auto_embed_text(embedder, texts[offset + idx],
                                                   &dims[idx]);
                }
            } else {
                for (size_t i = 0; i < need_count; i++) {
        ...
        for (size_t i = 0; i < chunk; i++) {
            ...
            if (metadata_keys && metadata_values &&
                ...
                rc = db_add_vector_with_metadata(db, vecs[i], dims[i],
...
int auto_embed_get_stats(const GV_AutoEmbedder *embedder,
