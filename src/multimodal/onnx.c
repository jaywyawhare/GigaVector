...
GV_ONNXModel *onnx_load(const GV_ONNXConfig *config) {
    if (!config || !config->model_path) {
        fprintf(stderr, "onnx_load: NULL config or model_path\n");
        return NULL;
    }
    ...
    for (size_t i = 0; i < m->input_count; i++) {
        ...
        m->input_names[i] = name ? gv_dup_cstr(name) : NULL;
    ...
    for (size_t i = 0; i < m->output_count; i++) {
        ...
        m->output_names[i] = name ? gv_dup_cstr(name) : NULL;
        if (name) allocator->Free(allocator, name);
    }
...
int onnx_rerank(GV_ONNXModel *model, const char *query_text,
    ...
    for (size_t base = 0; base < doc_count; base += batch) {
        ...
        for (size_t d = 0; d < chunk; d++) {
            ...
            GV_ONNXTensor input_ids  = onnx_tensor_create(in_shape, 2);
            GV_ONNXTensor attn_mask  = onnx_tensor_create(in_shape, 2);
            if (!input_ids.data || !attn_mask.data) {
                onnx_tensor_destroy(&input_ids);
                onnx_tensor_destroy(&attn_mask);
                rc = -1;
                break;
            }
            ...
            GV_ONNXTensor output = onnx_tensor_create(out_shape, 2);
            if (!output.data) {
                onnx_tensor_destroy(&input_ids);
                onnx_tensor_destroy(&attn_mask);
                rc = -1;
            ...
            if (onnx_infer(model, ins, 2, outs, 1) != 0) {
                onnx_tensor_destroy(&input_ids);
                onnx_tensor_destroy(&attn_mask);
    ...
    *input_names = gv_calloc(model->input_count, sizeof(char *));
    ...
    for (size_t i = 0; i < model->input_count; i++) {
        (*input_names)[i] = model->input_names[i]
                                ? gv_dup_cstr(model->input_names[i])
                                : NULL;
    }
    return 0;
}
    ...
    *output_names = gv_calloc(model->output_count, sizeof(char *));
    ...
    for (size_t i = 0; i < model->output_count; i++) {
        (*output_names)[i] = model->output_names[i]
                                 ? gv_dup_cstr(model->output_names[i])
                                 : NULL;
    }
    return 0;
}
...
GV_ONNXModel *onnx_load(const GV_ONNXConfig *config) {
    (void)config;
    fprintf(stderr,
            "onnx_load: ONNX Runtime not available "
