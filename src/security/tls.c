...
GV_TLSContext *tls_create(const GV_TLSConfig *config) {
    if (!config || !config->cert_file || !config->key_file) {
        fprintf(stderr, "tls: cert_file and key_file are required\n");
        return NULL;
    }
    ...
    ctx->ssl_ctx   = ssl_ctx;
    ctx->cert_path = gv_dup_cstr(config->cert_file);
...
int tls_accept(GV_TLSContext *ctx, int client_fd, void **tls_conn) {
    ...
    if (ret != 1) {
        ...
        fprintf(stderr, "tls: SSL_accept failed (ssl_err=%d)\n", err);
