...
int auth_generate_api_key(GV_AuthManager *auth, const char *description,
    ...
    entry->description = description ? gv_dup_cstr(description) : NULL;
...
int auth_list_api_keys(GV_AuthManager *auth, GV_APIKey **keys, size_t *count) {
    ...
    *keys = malloc(*count * sizeof(GV_APIKey));
    ...
    for (size_t i = 0; i < *count; i++) {
        (*keys)[i].key_id = gv_dup_cstr(auth->keys[i].key_id);
        (*keys)[i].key_hash = gv_dup_cstr(auth->keys[i].key_hash);
        (*keys)[i].description = auth->keys[i].description ? gv_dup_cstr(auth->keys[i].description) : NULL;
...
GV_AuthResult auth_verify_api_key(GV_AuthManager *auth, const char *api_key,
    ...
    auth_to_hex(hash, HASH_LEN, hash_hex);
...
GV_AuthResult auth_authenticate(GV_AuthManager *auth, const char *credential,
    ...
    if (auth->config.type == GV_AUTH_NONE) {
        if (identity) {
            memset(identity, 0, sizeof(*identity));
            identity->subject = gv_dup_cstr("anonymous");
            identity->auth_time = (uint64_t)time(NULL);
        }
        return GV_AUTH_SUCCESS;
    }
    ...
    if (strchr(credential, '.') != NULL) {
        return auth_verify_jwt(auth, credential, identity);
    } else {
        return auth_verify_api_key(auth, credential, identity);
    }
}
