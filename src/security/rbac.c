...
int rbac_create_role(GV_RBACManager *mgr, const char *role_name) {
    ...
    role->name = gv_dup_cstr(role_name);
    if (!role->name) {
        pthread_rwlock_unlock(&mgr->rwlock);
        return -1;
    }
...
int rbac_add_rule(GV_RBACManager *mgr, const char *role_name,
    ...
    role->rules[role->rule_count].resource = gv_dup_cstr(resource);
...
int rbac_remove_rule(GV_RBACManager *mgr, const char *role_name,
                         const char *resource) {
    if (!mgr || !role_name || !resource) return -1;
...
int rbac_set_inheritance(GV_RBACManager *mgr, const char *role_name,
...
int rbac_assign_role(GV_RBACManager *mgr, const char *user_id,
    ...
    if (!user) {
        ...
        user->user_id = gv_dup_cstr(user_id);
        if (!user->user_id) {
            pthread_rwlock_unlock(&mgr->rwlock);
            return -1;
        }
    ...
    user->role_names[user->role_count] = gv_dup_cstr(role_name);
...
int rbac_revoke_role(GV_RBACManager *mgr, const char *user_id,
                         const char *role_name) {
    if (!mgr || !user_id || !role_name) return -1;
...
int rbac_get_user_roles(const GV_RBACManager *mgr, const char *user_id,
    ...
    *out_roles = gv_alloc(user->role_count * sizeof(char *));
    ...
    for (size_t i = 0; i < user->role_count; i++) {
        (*out_roles)[i] = gv_dup_cstr(user->role_names[i]);
    }
...
int rbac_list_roles(const GV_RBACManager *mgr, char ***out_names,
    ...
    *out_names = gv_alloc(mgr->role_count * sizeof(char *));
    ...
    for (size_t i = 0; i < mgr->role_count; i++) {
        (*out_names)[i] = gv_dup_cstr(mgr->roles[i].name);
    }
