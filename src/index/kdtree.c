...
int kdtree_insert(GV_KDNode **root, GV_SoAStorage *storage, size_t vector_index, size_t depth) {
    ...
    if (*root == NULL) {
        ...
        GV_KDNode *node = kdtree_create_node(vector_index, axis);
        if (node == NULL) {
            return -1;
        }
        *root = node;
    ...
    const float *current_data = soa_storage_get_data(storage, current->vector_index);
    const float *point_data = soa_storage_get_data(storage, vector_index);
    ...
    if (point_value < current_value) {
        return kdtree_insert(&(current->left), storage, vector_index, depth + 1);
    }

    return kdtree_insert(&(current->right), storage, vector_index, depth + 1);
}
...
int kdtree_save_recursive(const GV_KDNode *node, const GV_SoAStorage *storage, FILE *out, uint32_t version) {
    ...
    const float *data = soa_storage_get_data(storage, node->vector_index);
    if (data == NULL) {
        return -1;
    }
    ...
    if (write_u32(out, (uint32_t)node->axis) != 0) {
        return -1;
    }

    if (write_floats(out, data, storage->dimension) != 0) {
        return -1;
    }

    if (version >= 2) {
        GV_Metadata *metadata = soa_storage_get_metadata(storage, node->vector_index);
        if (write_metadata(out, metadata) != 0) {
            return -1;
        }
    }

    if (kdtree_save_recursive(node->left, storage, out, version) != 0) {
        return -1;
    }

    return kdtree_save_recursive(node->right, storage, out, version);
}
...
int kdtree_load_recursive(GV_KDNode **root, GV_SoAStorage *storage, FILE *in, size_t dimension, uint32_t version) {
    ...
    GV_KDNode *node = kdtree_create_node(vector_index, (size_t)axis_u32);
    if (node == NULL) {
        return -1;
    }

    if (kdtree_load_recursive(&(node->left), storage, in, dimension, version) != 0) {
        kdtree_destroy_recursive(node);
        return -1;
    }

    if (kdtree_load_recursive(&(node->right), storage, in, dimension, version) != 0) {
        kdtree_destroy_recursive(node);
        return -1;
    }
...
void kdtree_destroy_recursive(GV_KDNode *node) {
    if (node == NULL) {
        return;
    }
...
static GV_Vector *knn_get_vector_view(GV_KNNStorageContext *storage_ctx, size_t index) {
    ...
    if (soa_storage_get_vector_view(storage_ctx->storage, index, &storage_ctx->temp_views[index]) != 0) {
        return NULL;
    }
...
static void knn_insert_result(GV_KNNContext *ctx, GV_KNNStorageContext *storage_ctx, size_t vector_index, float distance) {
    ...
    GV_Vector *vector = knn_get_vector_view(storage_ctx, vector_index);
    if (vector == NULL) {
        return;
    }
...
static int knn_check_metadata_filter(const GV_SoAStorage *storage, size_t vector_index, const char *key, const char *value) {
    ...
    GV_Metadata *metadata = soa_storage_get_metadata(storage, vector_index);
    if (metadata == NULL) {
        return 0;
    }
    GV_Vector temp_vec;
    ...
    temp_vec.data = NULL;
    temp_vec.metadata = metadata;
    const char *meta_value = vector_get_metadata(&temp_vec, key);
    if (meta_value == NULL) {
        return 0;
    }
...
static void knn_search_recursive(const GV_KDNode *node, const GV_SoAStorage *storage, const GV_Vector *query,
                                     GV_KNNContext *ctx, GV_KNNStorageContext *storage_ctx) {
    if (node == NULL || query == NULL || ctx == NULL || storage == NULL || storage_ctx == NULL) {
        return;
    }

    if (soa_storage_is_deleted(storage, node->vector_index) == 1) {
        /* Deleted node: skip this node but still recurse into children */
        goto recurse_children;
    }

    int metadata_match = knn_check_metadata_filter(storage, node->vector_index, ctx->filter_key, ctx->filter_value);

    const float *node_data = soa_storage_get_data(storage, node->vector_index);
    if (node_data == NULL) {
        return;
    }
    ...
    if (metadata_match) {
        ...
        node_vec.metadata = soa_storage_get_metadata(storage, node->vector_index);
...
recurse_children:
    ;
    const float *node_data_for_split = soa_storage_get_data(storage, node->vector_index);
    if (node_data_for_split == NULL) {
        /* Can't determine split, recurse both */
        if (node->left != NULL) {
            knn_search_recursive(node->left, storage, query, ctx, storage_ctx);
        }
        if (node->right != NULL) {
            knn_search_recursive(node->right, storage, query, ctx, storage_ctx);
        }
        return;
    }
...
int kdtree_knn_search(const GV_KDNode *root, const GV_SoAStorage *storage, const GV_Vector *query, size_t k,
...
int kdtree_knn_search_filtered(const GV_KDNode *root, const GV_SoAStorage *storage, const GV_Vector *query, size_t k,
                                   GV_SearchResult *results, GV_DistanceType distance_type,
...
static void range_insert_result(GV_RangeContext *ctx, GV_KNNStorageContext *storage_ctx, size_t vector_index, float distance) {
...
static void range_search_recursive(const GV_KDNode *node, const GV_SoAStorage *storage, const GV_Vector *query,
    ...
    if (!knn_check_metadata_filter(storage, node->vector_index, ctx->filter_key, ctx->filter_value)) {
        return;
    }
    ...
    if (dist <= ctx->radius) {
        range_insert_result(ctx, storage_ctx, node->vector_index, dist);
    }
    ...
    if (ctx->distance_type == GV_DISTANCE_EUCLIDEAN) {
        if (diff_sq <= ctx->radius * ctx->radius) {
            if (query_value < node_value) {
                range_search_recursive(node->left, storage, query, ctx, storage_ctx);
                range_search_recursive(node->right, storage, query, ctx, storage_ctx);
            } else {
                range_search_recursive(node->right, storage, query, ctx, storage_ctx);
                range_search_recursive(node->left, storage, query, ctx, storage_ctx);
            }
...
int kdtree_range_search(const GV_KDNode *root, const GV_SoAStorage *storage, const GV_Vector *query, float radius,
...
int kdtree_range_search_filtered(const GV_KDNode *root, const GV_SoAStorage *storage, const GV_Vector *query, float radius,
...
static GV_KDNode *kdtree_find_min(GV_KDNode *node, size_t axis, size_t target_axis, const GV_SoAStorage *storage) {
    if (node == NULL) {
        return NULL;
    }
    ...
    if (node->axis == target_axis) {
        ...
        return kdtree_find_min(node->left, (axis + 1) % storage->dimension, target_axis, storage);
    }

    GV_KDNode *best = node;
    const float *best_data = soa_storage_get_data(storage, node->vector_index);
    if (best_data == NULL) {
        return NULL;
    }

    GV_KDNode *left_min = kdtree_find_min(node->left, (axis + 1) % storage->dimension, target_axis, storage);
    if (left_min != NULL) {
        const float *left_data = soa_storage_get_data(storage, left_min->vector_index);
        if (left_data != NULL && left_data[target_axis] < best_data[target_axis]) {
            best = left_min;
        }
    }

    GV_KDNode *right_min = kdtree_find_min(node->right, (axis + 1) % storage->dimension, target_axis, storage);
    if (right_min != NULL) {
        const float *right_data = soa_storage_get_data(storage, right_min->vector_index);
        if (right_data != NULL) {
            const float *best_data_check = soa_storage_get_data(storage, best->vector_index);
...
static GV_KDNode *kdtree_delete_recursive(GV_KDNode *node, size_t vector_index, size_t depth, 
    ...
    if (node->vector_index == vector_index) {
        if (node->right != NULL) {
            GV_KDNode *min_node = kdtree_find_min(node->right, (axis + 1) % storage->dimension, axis, storage);
            if (min_node != NULL) {
            ...
            GV_KDNode *min_node = kdtree_find_min(node->left, (axis + 1) % storage->dimension, axis, storage);
            if (min_node != NULL) {
                node->vector_index = min_node->vector_index;
        ...
        const float *node_data = soa_storage_get_data(storage, node->vector_index);
        const float *target_data = soa_storage_get_data(storage, vector_index);
        if (node_data == NULL || target_data == NULL) {
            return node;
        }

        if (target_data[axis] < node_data[axis]) {
            node->left = kdtree_delete_recursive(node->left, vector_index, depth + 1, storage);
        } else {
            node->right = kdtree_delete_recursive(node->right, vector_index, depth + 1, storage);
        }
    }
...
int kdtree_delete(GV_KDNode **root, GV_SoAStorage *storage, size_t vector_index) {
    if (root == NULL || storage == NULL || vector_index >= storage->count) {
        return -1;
    }
    ...
    if (soa_storage_is_deleted(storage, vector_index) == 1) {
        return -1;
    }

    *root = kdtree_delete_recursive(*root, vector_index, 0, storage);
    if (soa_storage_mark_deleted(storage, vector_index) != 0) {
        return -1;
    }
