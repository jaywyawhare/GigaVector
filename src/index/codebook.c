...
GV_Codebook *codebook_load_fp(FILE *in) {
    ...
    GV_Codebook *cb = codebook_create((size_t)dimension, (size_t)m, nbits);
    if (!cb) return NULL;
...
GV_Codebook *codebook_load(const char *filepath) {
    ...
    GV_Codebook *cb = codebook_load_fp(f);
    fclose(f);
    return cb;
}
...
GV_Codebook *codebook_copy(const GV_Codebook *cb) {
    ...
    GV_Codebook *copy = codebook_create(cb->dimension, cb->m, cb->nbits);
    if (!copy) return NULL;
