...
static void sparse_postings_free(GV_SparsePosting **postings, size_t dimension) {
    if (!postings) return;
...
int sparse_index_load(GV_SparseIndex **index_out, FILE *in,
    ...
    GV_SparseIndex *idx = sparse_index_create(dimension);
    if (!idx) {
        return -1;
    }
    ...
    for (size_t v = 0; v < count; ++v) {
        ...
        if (sparse_read_metadata(in, sv) != 0) {
...
int sparse_index_update(GV_SparseIndex *index, size_t vector_index, GV_SparseVector *new_vector) {
    ...
    /* Replace the vector */
    sparse_vector_destroy(old_vector);
