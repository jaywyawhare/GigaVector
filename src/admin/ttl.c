...
int ttl_set(GV_TTLManager *mgr, size_t vector_index, uint64_t ttl_seconds) {
    ...
    if (ttl_seconds == 0) {
        return ttl_remove(mgr, vector_index);
    }

    uint64_t expire_at = current_time_unix() + ttl_seconds;
    return ttl_set_absolute(mgr, vector_index, expire_at);
}
...
static void *cleanup_thread_func(void *arg) {
    ...
    while (!mgr->cleanup_stop_requested) {
        ...
        if (rc == 0 || rc == 110 /* ETIMEDOUT */) {
            ...
            pthread_mutex_unlock(&mgr->mutex);
            ttl_cleanup_expired(mgr, mgr->cleanup_db);
            pthread_mutex_lock(&mgr->mutex);
        }
    }
...
int ttl_start_background_cleanup(GV_TTLManager *mgr, GV_Database *db) {
    if (!mgr || !db) return -1;
...
int ttl_set_bulk(GV_TTLManager *mgr, const size_t *indices, size_t count, uint64_t ttl_seconds) {
    ...
    for (size_t i = 0; i < count; i++) {
        if (ttl_set(mgr, indices[i], ttl_seconds) == 0) {
            success_count++;
        }
    }
