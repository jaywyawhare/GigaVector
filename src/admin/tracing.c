 ...
 */
static int trace_ensure_capacity(GV_QueryTrace *trace) {
 ...
 */
static GV_TraceSpan *trace_find_last_open_span(GV_QueryTrace *trace) {
...
void trace_end(GV_QueryTrace *trace) {
    if (!trace || !trace->active) {
        return;
    }

    if (trace->span_count > 0) {
        uint64_t now = trace_get_time_us();
...
void trace_destroy(GV_QueryTrace *trace) {
    if (!trace) {
        return;
    }
...
void trace_span_start(GV_QueryTrace *trace, const char *name) {
    ...
    if (trace_ensure_capacity(trace) != 0) {
        return;
    }

    GV_TraceSpan *span = &trace->spans[trace->span_count];
    span->name = gv_dup_cstr(name);
    span->start_us = trace_get_time_us();
    span->duration_us = 0;
    span->metadata = NULL;
...
void trace_span_end(GV_QueryTrace *trace) {
    if (!trace || !trace->active) {
        return;
    }

    GV_TraceSpan *span = trace_find_last_open_span(trace);
    if (!span) {
        return;
    }
...
void trace_span_add(GV_QueryTrace *trace, const char *name, uint64_t duration_us) {
    if (!trace || !trace->active || !name) {
        return;
    }
...
void trace_set_metadata(GV_QueryTrace *trace, const char *metadata) {
    if (!trace || !trace->active) {
        return;
    }
