...
void tt_destroy(GV_TimeTravelManager *mgr)
{
    if (!mgr)
        return;
...
uint64_t tt_record_update(GV_TimeTravelManager *mgr, size_t index,
...
uint64_t tt_record_delete(GV_TimeTravelManager *mgr, size_t index,
                              const float *vector, size_t dimension)
{
    if (!mgr || !vector || dimension == 0)
        return 0;
...
int tt_query_at_timestamp(const GV_TimeTravelManager *mgr, uint64_t timestamp,
                              size_t index, float *output, size_t dimension)
{
    if (!mgr || !output || dimension == 0)
        return -1;
...
uint64_t tt_current_version(const GV_TimeTravelManager *mgr)
{
    if (!mgr)
        return 0;
...
int tt_list_versions(const GV_TimeTravelManager *mgr, GV_VersionEntry *out,
                         size_t max_count)
{
...
GV_TimeTravelManager *tt_load(const char *path)
{
    if (!path)
        return NULL;
    ...
    /* Create the manager */
    GV_TimeTravelManager *mgr = tt_create(&config);
    if (!mgr)
        goto fail_early;
