...
static size_t hash_tenant_id(const char *id) {
    return hash_str(id) % HASH_BUCKETS;
}
...
void tiered_config_init(GV_TieredTenantConfig *config) {
    if (!config) return;
...
GV_TieredManager *tiered_create(const GV_TieredTenantConfig *config) {
    ...
    if (config) {
        ...
        tiered_config_init(&mgr->config);
    }

    return mgr;
}

void tiered_destroy(GV_TieredManager *mgr) {
    if (!mgr) return;
...
int tiered_remove_tenant(GV_TieredManager *mgr, const char *tenant_id) {
    if (!mgr || !tenant_id) return -1;
...
int tiered_get_info(const GV_TieredManager *mgr, const char *tenant_id,
                        GV_TenantInfo *info) {
...
int tiered_record_usage(GV_TieredManager *mgr, const char *tenant_id,
...
int tiered_check_promote(GV_TieredManager *mgr) {
    if (!mgr) return -1;
...
int tiered_list_tenants(const GV_TieredManager *mgr, GV_TenantTier tier,
...
size_t tiered_tenant_count(const GV_TieredManager *mgr) {
    if (!mgr) return 0;
...
GV_TieredManager *tiered_load(const char *path) {
    if (!path) return NULL;
