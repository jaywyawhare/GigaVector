...
void bloom_destroy(GV_BloomFilter *bf)
{
    if (bf == NULL) {
        return;
    }
...
int bloom_add_string(GV_BloomFilter *bf, const char *str)
{
    if (bf == NULL || str == NULL) {
        return -1;
    }
    return bloom_add(bf, str, strlen(str));
}

int bloom_check(const GV_BloomFilter *bf, const void *data, size_t len)
{
    if (bf == NULL || data == NULL) {
        return -1;
    }
...
int bloom_check_string(const GV_BloomFilter *bf, const char *str)
{
    ...
    return bloom_check(bf, str, strlen(str));
}

size_t bloom_count(const GV_BloomFilter *bf)
{
    if (bf == NULL) {
        return 0;
    }
...
double bloom_fp_rate(const GV_BloomFilter *bf)
{
    if (bf == NULL) {
        return 0.0;
    }
...
void bloom_clear(GV_BloomFilter *bf)
{
    if (bf == NULL) {
        return;
    }
...
int bloom_load(GV_BloomFilter **bf_ptr, FILE *in)
{
...
GV_BloomFilter *bloom_merge(const GV_BloomFilter *a, const GV_BloomFilter *b)
{
    if (a == NULL || b == NULL) {
        return NULL;
    }
