        ...
        if (bracket) {
            ...
            while (*p) {
                ...
                *end_bracket = '\0';
                ...
                current = json_array_get(current, (size_t)idx_val);
                if (!current) return NULL;
            ...
            if (current->type == GV_JSON_OBJECT) {
                ...
                /* Allow numeric token as array index (compat with json_get_path) */
                char *endptr;
...
void json_index_destroy(GV_JSONPathIndex *idx) {
    if (!idx) return;
...
int json_index_remove_path(GV_JSONPathIndex *idx, const char *path) {
...
int json_index_insert(GV_JSONPathIndex *idx, size_t vector_index, const char *json_str) {
    ...
    GV_JsonError err;
    GV_JsonValue *root = json_parse(json_str, &err);
    if (!root) return -1;
    ...
    for (size_t i = 0; i < idx->path_count; i++) {
        ...
        switch (pi->type) {
            ...
            case GV_JP_INT: {
                double num;
                if (json_get_number(val, &num) == GV_JSON_OK) {
            ...
            case GV_JP_BOOL: {
                bool b;
                if (json_get_bool(val, &b) == GV_JSON_OK) {
...
int json_index_remove(GV_JSONPathIndex *idx, size_t vector_index) {
    if (!idx) return -1;
...
int json_index_lookup_int_range(const GV_JSONPathIndex *idx, const char *path,
...
int json_index_lookup_float_range(const GV_JSONPathIndex *idx, const char *path,
...
int json_index_save(const GV_JSONPathIndex *idx, const char *path_file) {
...
GV_JSONPathIndex *json_index_load(const char *path_file) {
