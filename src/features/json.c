...
GV_JsonValue *json_string(const char *value) {
    if (value == NULL) {
        return NULL;
    }
    GV_JsonValue *val = (GV_JsonValue *)gv_calloc(1, sizeof(GV_JsonValue));
...
GV_JsonValue *json_copy(const GV_JsonValue *value) {
    ...
    switch (value->type) {
        case GV_JSON_NULL:
            ...
            break;
        case GV_JSON_BOOL:
            copy = json_bool(value->data.boolean);
            break;
        case GV_JSON_NUMBER:
            copy = json_number(value->data.number);
            break;
        case GV_JSON_STRING:
            copy = json_string(value->data.string);
            break;
        case GV_JSON_ARRAY:
            copy = json_array();
            if (copy) {
                for (size_t i = 0; i < value->data.array.count; i++) {
                    GV_JsonValue *item_copy = json_copy(value->data.array.items[i]);
                    if (item_copy == NULL || json_array_push(copy, item_copy) != GV_JSON_OK) {
                        json_free(item_copy);
                        json_free(copy);
                        return NULL;
                    }
        ...
        case GV_JSON_OBJECT:
            ...
            if (copy) {
                for (size_t i = 0; i < value->data.object.count; i++) {
                    GV_JsonValue *val_copy = json_copy(value->data.object.entries[i].value);
                    if (val_copy == NULL ||
                        json_object_set(copy, value->data.object.entries[i].key, val_copy) != GV_JSON_OK) {
                        json_free(val_copy);
                        json_free(copy);
                        return NULL;
                    }
...
GV_JsonError json_get_number(const GV_JsonValue *value, double *out) {
    if (value == NULL || out == NULL) {
        return GV_JSON_ERROR_NULL_INPUT;
    }
...
const char *json_get_string(const GV_JsonValue *value) {
...
GV_JsonValue *json_array_get(const GV_JsonValue *array, size_t index) {
...
size_t json_array_length(const GV_JsonValue *array) {
    if (array == NULL || array->type != GV_JSON_ARRAY) {
        return 0;
    }
...
GV_JsonValue *json_object_get(const GV_JsonValue *object, const char *key) {
...
bool json_object_has(const GV_JsonValue *object, const char *key) {
...
GV_JsonValue *json_get_path(const GV_JsonValue *root, const char *path) {
    ...
    char *path_copy = gv_dup_cstr(path);
    if (path_copy == NULL) {
        return NULL;
    }
...
const char *json_get_string_path(const GV_JsonValue *root, const char *path) {
...
static GV_JsonValue *parse_array(ParserState *state) {
    ...
    while (1) {
        ...
        if (json_array_push(array, element) != GV_JSON_OK) {
            json_free(element);
            json_free(array);
            state->error = GV_JSON_ERROR_MEMORY;
            state->depth--;
...
static GV_JsonValue *parse_object(ParserState *state) {
    ...
    GV_JsonValue *object = json_object();
    if (object == NULL) {
        state->error = GV_JSON_ERROR_MEMORY;
        state->depth--;
        return NULL;
...
GV_JsonValue *json_parse(const char *json_str, GV_JsonError *error) {
    if (json_str == NULL) {
