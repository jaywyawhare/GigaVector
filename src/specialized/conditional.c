                                                ...
                                                const GV_Condition *cond)
{
    ...
    case GV_COND_METADATA_EQ: {
        ...
        GV_Metadata *meta = soa_storage_get_metadata(soa, index);
    ...
    case GV_COND_NOT_DELETED: {
        int deleted = soa_storage_is_deleted(soa, index);
                                              ...
                                              size_t condition_count)
{
    ...
    /* Apply: soa_storage_update_metadata transfers ownership of new_list */
    if (soa_storage_update_metadata(db->soa_storage, index, new_list) != 0) {
        metadata_free(new_list);
        pthread_rwlock_unlock(&db->rwlock);
        return GV_COND_FAILED;
    }
                          ...
                          GV_ConditionalResult *results)
{
     ...
     * validated by soa_storage_update_data against the SoA storage. */
    (void)dimension;
                                                ...
                                                uint64_t expected_version)
{
    ...
    return cond_update_vector(mgr, index, new_embedding, dimension, &cond, 1);
}
