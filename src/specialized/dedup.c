...
void dedup_destroy(GV_DedupIndex *dedup) {
    if (dedup == NULL) {
        return;
    }
...
int dedup_insert(GV_DedupIndex *dedup, const float *data, size_t dimension) {
    if (dedup == NULL || data == NULL) {
        return -1;
    }
    if (dimension != dedup->dimension) {
...
int dedup_scan(GV_DedupIndex *dedup, GV_DedupResult *results, size_t max_results) {
...
size_t dedup_count(const GV_DedupIndex *dedup) {
    if (dedup == NULL) {
        return 0;
    }
    return dedup->count;
...
void dedup_clear(GV_DedupIndex *dedup) {
    if (dedup == NULL) {
        return;
    }
