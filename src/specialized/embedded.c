...
GV_HEAP_DEFINE(emb_heap, GV_EmbeddedHeapItem)
...
static int emb_hnsw_insert(GV_EmbeddedDB *db, size_t vec_idx) {
    ...
    if (ef > 0) {
        ...
        for (size_t i = 0; i < db->count; ++i) {
            ...
            emb_heap_push(candidates, &cand_count, ef, (GV_EmbeddedHeapItem){dist, i});
        }
    }
...
static int emb_hnsw_search(const GV_EmbeddedDB *db, const float *query, size_t k,
    ...
    emb_heap_push(heap, heap_size, k, (GV_EmbeddedHeapItem){entry_dist, entry});
    ...
    while (cursor < cand_size) {
        ...
        for (size_t n = 0; n < node->neighbor_count; ++n) {
            ...
            float dist = emb_compute_distance(query, db->vectors + nb_idx * db->dimension,
                                               db->dimension, dist_type);
            emb_heap_push(heap, heap_size, k, (GV_EmbeddedHeapItem){dist, nb_idx});
    ...
    for (size_t t = 0; t < l->num_tables; ++t) {
        ...
        for (size_t i = 0; i < bucket->count; ++i) {
            ...
            float dist = emb_compute_distance(query, db->vectors + vec_idx * db->dimension,
                                               db->dimension, dist_type);
            emb_heap_push(heap, heap_size, k, (GV_EmbeddedHeapItem){dist, vec_idx});
        }
    }
...
void embedded_config_init(GV_EmbeddedConfig *config) {
    if (!config) return;
...
GV_EmbeddedDB *embedded_open(const GV_EmbeddedConfig *config) {
...
void embedded_close(GV_EmbeddedDB *db) {
    if (!db) return;
...
int embedded_add_with_id(GV_EmbeddedDB *db, size_t id, const float *vector) {
    if (!db || !vector) return -1;
...
int embedded_search(const GV_EmbeddedDB *db, const float *query, size_t k,
    ...
    if (db->index_type == GV_EMBEDDED_INDEX_HNSW && db->hnsw) {
        ...
        for (size_t i = 0; i < db->count; ++i) {
            ...
            emb_heap_push(heap, &heap_size, k, (GV_EmbeddedHeapItem){dist, i});
        }
    }
...
int embedded_delete(GV_EmbeddedDB *db, size_t index) {
    if (!db) return -1;
...
int embedded_get(const GV_EmbeddedDB *db, size_t index, float *output) {
...
size_t embedded_count(const GV_EmbeddedDB *db) {
    if (!db) return 0;
...
size_t embedded_memory_usage(const GV_EmbeddedDB *db) {
    if (!db) return 0;
    return db->memory_used;
}


int embedded_save(const GV_EmbeddedDB *db, const char *path) {
    if (!db || !path) return -1;
    ...
    if (write_u32(f, GV_EMBEDDED_FILE_VERSION) != 0) { fclose(f); return -1; }
    if (write_u64(f, (uint64_t)db->dimension) != 0)  { fclose(f); return -1; }
    if (write_u64(f, (uint64_t)db->count) != 0)      { fclose(f); return -1; }
    if (write_u32(f, (uint32_t)db->index_type) != 0)  { fclose(f); return -1; }
    if (write_u32(f, (uint32_t)db->quantize) != 0)    { fclose(f); return -1; }
    if (write_u64(f, (uint64_t)db->max_vectors) != 0) { fclose(f); return -1; }
    if (write_u64(f, (uint64_t)db->memory_limit_mb) != 0) { fclose(f); return -1; }
    ...
    if (db->index_type == GV_EMBEDDED_INDEX_HNSW && db->hnsw) {
        if (write_u64(f, (uint64_t)db->hnsw->M) != 0) { fclose(f); return -1; }
        if (write_u64(f, (uint64_t)db->hnsw->ef_construction) != 0) { fclose(f); return -1; }
        ...
        for (size_t i = 0; i < db->count; ++i) {
            ...
            if (write_u32(f, nc) != 0) { fclose(f); return -1; }
            for (uint32_t n = 0; n < nc; ++n) {
                if (write_u64(f, (uint64_t)node->neighbors[n]) != 0) { fclose(f); return -1; }
            }
        }
    }

    if (db->index_type == GV_EMBEDDED_INDEX_LSH && db->lsh) {
        if (write_u64(f, (uint64_t)db->lsh->num_tables) != 0) { fclose(f); return -1; }
        if (write_u64(f, (uint64_t)db->lsh->num_bits) != 0)   { fclose(f); return -1; }
        if (write_u64(f, (uint64_t)db->lsh->num_buckets) != 0){ fclose(f); return -1; }
...
GV_EmbeddedDB *embedded_load(const char *path) {
    if (!path) return NULL;
    ...
    uint32_t version = 0;
    if (read_u32(f, &version) != 0 || version != GV_EMBEDDED_FILE_VERSION) {
        fclose(f);
        return NULL;
    }
    ...
    if (read_u64(f, &dim64) != 0)       { fclose(f); return NULL; }
    if (read_u64(f, &count64) != 0)      { fclose(f); return NULL; }
    if (read_u32(f, &idx_type) != 0)     { fclose(f); return NULL; }
    if (read_u32(f, &quant) != 0)        { fclose(f); return NULL; }
    if (read_u64(f, &max_vec64) != 0)    { fclose(f); return NULL; }
    if (read_u64(f, &mem_limit64) != 0)  { fclose(f); return NULL; }

    GV_EmbeddedConfig config;
    embedded_config_init(&config);
    ...
    GV_EmbeddedDB *db = embedded_open(&config);
    ...
    if (db->index_type == GV_EMBEDDED_INDEX_HNSW && db->hnsw && count > 0) {
        ...
        if (read_u64(f, &M64) != 0 || read_u64(f, &ef64) != 0) {
            embedded_close(db);
            fclose(f);
            return NULL;
        }
    ...
    if (db->index_type == GV_EMBEDDED_INDEX_LSH && db->lsh && count > 0) {
        ...
        if (read_u64(f, &nt64) != 0 || read_u64(f, &nb64) != 0 ||
            read_u64(f, &nbk64) != 0) {
            embedded_close(db);
            fclose(f);
            return NULL;
...
int embedded_compact(GV_EmbeddedDB *db) {
    if (!db) return -1;
