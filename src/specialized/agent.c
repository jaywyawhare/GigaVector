 ...
 *
 * Thread-safe via pthread_mutex_t on the agent handle.  Uses llm.h for
 * LLM calls and json.h for structured response parsing.
 */
...
static GV_AgentResult *result_error(GV_AgentResult *r, const char *fmt, ...) {
    ...
    r->error_message = gv_dup_cstr(buf);
    return r;
}
...
static char *llm_call_with_retry(GV_Agent *agent, const char *system_prompt,
    ...
    messages[0].role = gv_dup_cstr("system");
    messages[0].content = gv_dup_cstr(system_prompt);
    messages[1].role = gv_dup_cstr("user");
    messages[1].content = gv_dup_cstr(user_message);
    ...
    for (int i = 0; i < attempts; i++) {
        ...
        int rc = llm_generate_response(agent->llm, messages, 2, "json_object", &response);
        if (rc == GV_LLM_SUCCESS && response.content != NULL) {
            content = gv_dup_cstr(response.content);
...
static double json_get_number_or(const GV_JsonValue *obj, const char *key, double fallback) {
    GV_JsonValue *val = json_object_get(obj, key);
    if (val == NULL || !json_is_number(val)) return fallback;
    double out;
    if (json_get_number(val, &out) != GV_JSON_OK) return fallback;
    return out;
}
...
static const char *json_get_string_or_null(const GV_JsonValue *obj, const char *key) {
    GV_JsonValue *val = json_object_get(obj, key);
    if (val == NULL || !json_is_string(val)) return NULL;
...
void agent_destroy(GV_Agent *agent) {
...
GV_AgentResult *agent_query(GV_Agent *agent, const char *natural_language_query, size_t k) {
    ...
    if (query_vec == NULL) {
        GV_JsonValue *vec_val = json_object_get(root, "search_vector");
            ...
            if (vec_len > 0 && vec_len == agent->db->dimension) {
                ...
                if (query_vec != NULL) {
                    ...
                    for (size_t i = 0; i < vec_len; i++) {
                        GV_JsonValue *elem = json_array_get(vec_val, i);
                        double v = 0.0;
                        if (elem && json_is_number(elem)) {
                            json_get_number(elem, &v);
                        }
                        query_vec[i] = (float)v;
                    }
                }
    ...
    if (query_vec != NULL && embed_dim == agent->db->dimension) {
        ...
        if (result->generated_filter != NULL) {
            found = db_search_with_filter_expr(agent->db, query_vec, effective_k,
...
GV_AgentResult *agent_transform(GV_Agent *agent, const char *natural_language_instruction) {
    ...
    GV_Filter *filter = filter_parse(filter_str);
    if (filter == NULL) {
        json_free(root);
        pthread_mutex_unlock(&agent->mutex);
    ...
    if (strcmp(operation, "delete") == 0) {
        ...
        if (match_indices != NULL) {
            for (size_t i = 0; i < total; i++) {
                ...
                if (filter_eval(filter, &tmp_vec) == 1) {
            ...
            for (size_t j = match_count; j > 0; j--) {
                if (db_delete_vector_by_index(agent->db, match_indices[j - 1]) == 0) {
                    deleted++;
                }
            }
                ...
                if (n_keys > 0) {
                    ...
                    if (keys && vals && val_copies) {
                        ...
                        for (size_t e = 0; e < update_obj->data.object.count && ki < n_keys; e++) {
                            ...
                            const char *sv = json_get_string(update_obj->data.object.entries[e].value);
                            if (sv != NULL) {
                                val_copies[ki] = gv_dup_cstr(sv);
                                vals[ki] = val_copies[ki];
                            } else {
                                /* For non-string values, stringify */
                                char *s = json_stringify(update_obj->data.object.entries[e].value, 0);
                                val_copies[ki] = s;
                                vals[ki] = s ? s : "";
                            }
                            ki++;
                        ...
                        if (db_update_vector_metadata(agent->db, i, keys, vals, ki) == 0) {
                            updated++;
                        }
...
GV_AgentResult *agent_personalize(GV_Agent *agent, const char *query,
    ...
    int found;
    if (result->generated_filter != NULL) {
        found = db_search_with_filter_expr(agent->db, query_vec, oversample_k,
                                               sr, dist_type, result->generated_filter);
    } else {
    ...
    for (int i = 0; i < found; i++) {
        ...
        if (adjustments != NULL && json_is_array(adjustments)) {
            ...
            for (size_t a = 0; a < adj_count; a++) {
                GV_JsonValue *adj = json_array_get(adjustments, a);
                if (adj == NULL || !json_is_object(adj)) continue;
