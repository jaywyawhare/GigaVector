 ...
 * When compiled with CUDA support, gpu_cuda.cu provides GPU implementations.
 */
...
void gpu_config_init(GV_GPUConfig *config) {
    if (!config) return;
...
int gpu_get_device_info(int device_id, GV_GPUDeviceInfo *info) {
    if (!info) return -1;
...
void gpu_destroy(GV_GPUContext *ctx) {
    if (!ctx) return;
...
GV_GPUIndex *gpu_index_from_db(GV_GPUContext *ctx, GV_Database *db) {
...
int gpu_index_add(GV_GPUIndex *index, const float *vectors, size_t count) {
...
int gpu_index_update(GV_GPUIndex *index, const size_t *indices,
...
int gpu_index_info(GV_GPUIndex *index, size_t *count, size_t *dimension,
                       size_t *memory_usage) {
    if (!index) return -1;
...
void gpu_index_destroy(GV_GPUIndex *index) {
    if (!index) return;
...
int gpu_index_compute_distances(GV_GPUIndex *index, const float *queries,
                                    size_t num_queries,
                                    GV_GPUDistanceMetric metric,
                                    float *distances) {
    if (!index || !queries || !distances) return -1;

    return gpu_compute_distances(index->ctx, queries, num_queries,
...
int gpu_knn_search(GV_GPUContext *ctx, const float *queries,
                       size_t num_queries, const float *database,
                       size_t num_vectors, size_t dimension,
                       const GV_GPUSearchParams *params,
                       size_t *indices, float *distances) {
...
int gpu_index_knn_search(GV_GPUIndex *index, const float *queries,
                             size_t num_queries, const GV_GPUSearchParams *params,
                             size_t *indices, float *distances) {
    if (!index || !queries || !params || !indices || !distances) return -1;

    return gpu_knn_search(index->ctx, queries, num_queries,
                              index->vectors, index->count,
                              index->dimension, params,
                              indices, distances);
}

int gpu_index_search(GV_GPUIndex *index, const float *query,
                         const GV_GPUSearchParams *params,
                         size_t *indices, float *distances) {
...
int gpu_batch_search(GV_GPUContext *ctx, GV_Database *db,
...
int gpu_reset_stats(GV_GPUContext *ctx) {
    if (!ctx) return -1;
