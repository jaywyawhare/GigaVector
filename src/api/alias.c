...
int alias_update(GV_AliasManager *mgr, const char *alias_name,
...
int alias_list(const GV_AliasManager *mgr, GV_AliasInfo **out_list,
    ...
    *out_list = gv_calloc(mgr->count, sizeof(GV_AliasInfo));
    ...
    for (size_t i = 0; i < MAX_ALIASES && pos < mgr->count; i++) {
        ...
        (*out_list)[pos].alias_name      = gv_dup_cstr(e->alias_name);
        (*out_list)[pos].collection_name = gv_dup_cstr(e->collection_name);
...
int alias_get_info(const GV_AliasManager *mgr, const char *alias_name,
    ...
    const GV_AliasEntry *e = &mgr->entries[idx];
    info->alias_name      = gv_dup_cstr(e->alias_name);
    info->collection_name = gv_dup_cstr(e->collection_name);
...
int alias_save(const GV_AliasManager *mgr, const char *filepath) {
    ...
    if (write_u32(fp, (uint32_t)mgr->count) != 0) {
        fclose(fp);
        pthread_rwlock_unlock((pthread_rwlock_t *)&mgr->rwlock);
        return -1;
    }
