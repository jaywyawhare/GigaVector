...
GV_TypedValue typed_string(const char *str) {
    GV_TypedValue val;
    memset(&val, 0, sizeof(val));
    val.type = GV_META_TYPE_STRING;
    if (str) {
        val.data.string_val = gv_dup_cstr(str);
    }
    return val;
}

GV_TypedValue typed_int(int64_t i) {
    GV_TypedValue val;
    memset(&val, 0, sizeof(val));
...
GV_TypedValue typed_float(double f) {
    GV_TypedValue val;
    memset(&val, 0, sizeof(val));
...
GV_TypedValue typed_bool(bool b) {
    GV_TypedValue val;
    memset(&val, 0, sizeof(val));
...
bool typed_object_has(const GV_TypedValue *object, const char *key) {
    return typed_object_get(object, key) != NULL;
}
    ...
    switch (a->type) {
        ...
        case GV_META_TYPE_ARRAY: {
            ...
            for (size_t i = 0; i < a->data.array_val.count; i++) {
                if (!typed_equals(&a->data.array_val.items[i], &b->data.array_val.items[i])) {
                    return false;
                }
            }
            return true;
        ...
        case GV_META_TYPE_OBJECT: {
            ...
            for (size_t i = 0; i < a->data.object_val.count; i++) {
                GV_TypedValue *bval = typed_object_get(b, a->data.object_val.keys[i]);
                if (!bval || !typed_equals(&a->data.object_val.values[i], bval)) {
                    return false;
                }
            }
            return true;
...
bool typed_array_contains(const GV_TypedValue *array, const GV_TypedValue *item) {
    ...
    for (size_t i = 0; i < array->data.array_val.count; i++) {
        if (typed_equals(&array->data.array_val.items[i], item)) {
            return true;
        }
    }
    return false;
...
GV_TypedValue typed_value_copy(const GV_TypedValue *src) {
    ...
    switch (src->type) {
        case GV_META_TYPE_NULL:
            break;
        case GV_META_TYPE_STRING:
            dst.data.string_val = src->data.string_val ? gv_dup_cstr(src->data.string_val) : NULL;
            break;
...
char *typed_to_string(const GV_TypedValue *value) {
    ...
    switch (value->type) {
        case GV_META_TYPE_NULL:
            return gv_dup_cstr("null");
        case GV_META_TYPE_STRING:
            return value->data.string_val ? gv_dup_cstr(value->data.string_val) : gv_dup_cstr("");
        case GV_META_TYPE_INT64:
        ...
        case GV_META_TYPE_FLOAT64:
            ...
            return gv_dup_cstr(buf);
        case GV_META_TYPE_BOOL:
            return gv_dup_cstr(value->data.bool_val ? "true" : "false");
        case GV_META_TYPE_ARRAY:
