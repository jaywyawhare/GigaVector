...
static void rate_limiter_init(GV_RateLimiter *rl, double rate, size_t burst) {
...
static void rate_limiter_destroy(GV_RateLimiter *rl) {
 ...
 */
static int rate_limiter_allow(GV_RateLimiter *rl) {
...
int server_start(GV_Server *server) {
    if (!server) {
...
int server_stop(GV_Server *server) {
    if (!server) {
        return GV_SERVER_ERROR_NULL_POINTER;
    }
...
void server_destroy(GV_Server *server) {
    if (!server) {
        return;
    }
...
uint16_t server_get_port(const GV_Server *server) {
