...
void mmr_config_init(GV_MMRConfig *config) {
    if (!config) return;
 ...
 * so they can be passed to the existing distance() API.
 */
...
int mmr_search(const void *db_ptr, const float *query, size_t dimension,
    ...
    if (dimension != database_dimension(db)) return -1;
    ...
    /* Fetch oversampled candidates via db_search */
    ...
    /* Apply MMR reranking */
    int result_count = mmr_rerank(query, dimension,
