...
void hybrid_config_init(GV_HybridConfig *config) {
    if (!config) return;
...
double hybrid_rrf_fusion(size_t vector_rank, size_t text_rank, double k) {
    double score = 0.0;
    if (vector_rank > 0) {
...
double hybrid_normalize_score(double score, double min_score, double max_score) {
...
int hybrid_search(GV_HybridSearcher *searcher, const float *query_vector,
    ...
    return hybrid_search_with_stats(searcher, query_vector, query_text, k,
                                        results, NULL);
}

int hybrid_search_with_stats(GV_HybridSearcher *searcher, const float *query_vector,
    ...
    for (size_t i = 0; i < candidate_count; i++) {
        ...
        if (query_vector && c->vector_rank > 0) {
            norm_vec = hybrid_normalize_score(c->vector_score, vec_min, vec_max);
        }
        if (query_text && c->text_rank > 0) {
            norm_txt = hybrid_normalize_score(c->text_score, txt_min, txt_max);
        }

        switch (cfg->fusion_type) {
            case GV_FUSION_LINEAR:
                c->combined_score = hybrid_linear_fusion(norm_vec, norm_txt,
                                                             cfg->vector_weight, cfg->text_weight);
                break;

            case GV_FUSION_RRF:
                c->combined_score = hybrid_rrf_fusion(c->vector_rank, c->text_rank, cfg->rrf_k);
                break;
...
int hybrid_search_vector_only(GV_HybridSearcher *searcher, const float *query_vector,
                                  size_t k, GV_HybridResult *results) {
    return hybrid_search(searcher, query_vector, NULL, k, results);
}

int hybrid_search_text_only(GV_HybridSearcher *searcher, const char *query_text,
                                size_t k, GV_HybridResult *results) {
...
int hybrid_get_config(const GV_HybridSearcher *searcher, GV_HybridConfig *config) {
    if (!searcher || !config) return -1;
...
int hybrid_set_weights(GV_HybridSearcher *searcher, double vector_weight,
                           double text_weight) {
