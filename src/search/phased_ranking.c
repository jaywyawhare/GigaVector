/**
 * @file phased_ranking.c
 * @brief Multi-stage phased ranking pipeline implementation.
 *
 * Executes a configurable sequence of ranking phases:
 *   1. ANN -- fast approximate nearest-neighbor retrieval via db_search().
 *   2. RERANK_EXPR -- expression-based reranking via ranking.h.
 *   3. RERANK_MMR  -- diversity reranking via mmr.h.
 *   4. RERANK_CALLBACK -- user-supplied scoring function.
 *   5. FILTER -- metadata filter expression via filter.h.
 *
...
void pipeline_clear_phases(GV_Pipeline *pipe) {
    if (!pipe) return;
...
size_t pipeline_phase_count(const GV_Pipeline *pipe) {
    if (!pipe) return 0;
...
static int execute_rerank_expr_phase(const GV_PhaseConfig *config,
    ...
    GV_RankExpr *expr = rank_expr_parse(config->params.expr.expression);
    if (!expr) return -1;

    for (size_t i = 0; i < count; i++) {
        float vector_score = candidates[i].score;
        double new_score = rank_expr_eval(expr, vector_score, NULL, 0);
 ...
 *
 * Uses mmr_rerank() to select diverse candidates from the current set.
 */
static int execute_rerank_mmr_phase(const GV_Database *db,
    ...
    size_t valid = 0;
    for (size_t i = 0; i < count; i++) {
        const float *vec = database_get_vector(db, candidates[i].index);
        if (!vec) continue;
    ...
    int mmr_count = mmr_rerank(query, dimension,
...
static int execute_filter_phase(const GV_Database *db,
    ...
    GV_Filter *filter = filter_parse(config->params.filter.filter_expr);
    if (!filter) return -1;
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        if (storage) {
            soa_storage_get_vector_view(storage, candidates[i].index, &view);
        }

        int match = filter_eval(filter, &view);
        if (match == 1) {
...
int pipeline_execute(GV_Pipeline *pipe, const float *query,
    ...
    if (dimension != database_dimension(pipe->db)) {
        pthread_mutex_unlock(&pipe->mutex);
        return -1;
    }
...
int pipeline_get_stats(const GV_Pipeline *pipe, GV_PipelineStats *stats) {
