/*
 * filter_ops.c -- Bulk delete/update operations driven by filter expressions.
 *
 * Every public function follows the same pattern:
 *   1. Parse the filter expression with filter_parse().
 *   2. Iterate over all vectors in the SoA storage.
 *   3. Skip deleted vectors.
 *   4. Build a lightweight GV_Vector view for the filter evaluator.
 *   5. Evaluate the filter; on match, perform the requested operation.
 *   6. Clean up and return the match count (or -1 on error).
 */

#include <stdlib.h>
#include "core/memory.h"
#include <string.h>

#include "search/filter_ops.h"
#include "core/scope.h"
#include "storage/database.h"
#include "search/filter.h"
#include "core/types.h"
#include "storage/soa_storage.h"

/* Internal helper: collect all non-deleted indices that match a */
/* compiled filter.  Returns match count, or -1 on error.  When */
/* out_indices is NULL the function just counts. */
static int filter_ops_collect_matches(const GV_Database *db,
                                         const GV_Filter *filter,
                                         size_t *out_indices,
                                         size_t max_count)
{
    if (!db || !filter) {
        return -1;
    }

    const GV_SoAStorage *storage = db->soa_storage;
    if (!storage) {
        return -1;
    }

    size_t total = soa_storage_count(storage);
    int matched = 0;

    for (size_t i = 0; i < total; i++) {
        /* Skip deleted vectors. */
        if (soa_storage_is_deleted(storage, i) == 1) {
            continue;
        }

        /* Build a temporary GV_Vector view for the filter evaluator. */
        GV_Vector view;
        if (soa_storage_get_vector_view(storage, i, &view) != 0) {
            continue;
        }

        int result = filter_eval(filter, &view);
        if (result == 1) {
            if (out_indices != NULL && (size_t)matched < max_count) {
                out_indices[matched] = i;
            }
            matched++;
        }
    }

    return matched;
}

static size_t *filter_ops_alloc_indices(size_t count, int *on_heap) {
    size_t *indices = (size_t *)gv_tls_alloc(sizeof(size_t) * count, sizeof(size_t));
    if (indices != NULL) {
        *on_heap = 0;
        return indices;
    }
    *on_heap = 1;
    return (size_t *)gv_alloc(sizeof(size_t) * count);
}

static void filter_ops_free_indices(size_t *indices, int on_heap) {
    if (indices != NULL && on_heap) {
        gv_free(indices);
    }
}

/* Public API */

int db_delete_by_filter(GV_Database *db, const char *filter_expr)
{
    if (!db || !filter_expr) {
        return -1;
    }

    GV_Filter *filter = filter_parse(filter_expr);
    if (!filter) {
        return -1;
    }

    /* First pass: count matches so we can allocate an index buffer. */
    int count = filter_ops_collect_matches(db, filter, NULL, 0);
    if (count <= 0) {
        filter_destroy(filter);
        return count; /* 0 matches or error */
    }

    int on_heap = 0;
    size_t *indices = filter_ops_alloc_indices((size_t)count, &on_heap);
    if (!indices) {
        filter_destroy(filter);
        return -1;
    }

    /* Second pass: collect the actual indices. */
    int collected = filter_ops_collect_matches(db, filter, indices, (size_t)count);
    filter_destroy(filter);

    if (collected < 0) {
        filter_ops_free_indices(indices, on_heap);
        return -1;
    }

    /*
     * Delete in reverse order so that earlier indices remain valid when
     * the underlying storage merely marks entries as deleted (which is
     * the case for SoA storage).  This also guards against any future
     * implementation that compacts on delete.
     */
    int deleted = 0;
    for (int i = collected - 1; i >= 0; i--) {
        if (db_delete_vector_by_index(db, indices[i]) == 0) {
            deleted++;
        }
    }

    filter_ops_free_indices(indices, on_heap);
    return deleted;
}

int db_update_by_filter(GV_Database *db, const char *filter_expr,
                            const float *new_data, size_t dimension)
{
    if (!db || !filter_expr || !new_data || dimension == 0) {
        return -1;
    }

    GV_Filter *filter = filter_parse(filter_expr);
    if (!filter) {
        return -1;
    }

    /* First pass: count. */
    int count = filter_ops_collect_matches(db, filter, NULL, 0);
    if (count <= 0) {
        filter_destroy(filter);
        return count;
    }

    int on_heap = 0;
    size_t *indices = filter_ops_alloc_indices((size_t)count, &on_heap);
    if (!indices) {
        filter_destroy(filter);
        return -1;
    }

    /* Second pass: collect indices. */
    int collected = filter_ops_collect_matches(db, filter, indices, (size_t)count);
    filter_destroy(filter);

    if (collected < 0) {
        filter_ops_free_indices(indices, on_heap);
        return -1;
    }

    int updated = 0;
    for (int i = 0; i < collected; i++) {
        if (db_update_vector(db, indices[i], new_data, dimension) == 0) {
            updated++;
        }
    }

    filter_ops_free_indices(indices, on_heap);
    return updated;
}

int db_update_metadata_by_filter(GV_Database *db, const char *filter_expr,
                                     const char *const *metadata_keys,
                                     const char *const *metadata_values,
                                     size_t metadata_count)
{
    if (!db || !filter_expr) {
        return -1;
    }
    if (metadata_count > 0 && (!metadata_keys || !metadata_values)) {
        return -1;
    }

    GV_Filter *filter = filter_parse(filter_expr);
    if (!filter) {
        return -1;
    }

    /* First pass: count. */
    int count = filter_ops_collect_matches(db, filter, NULL, 0);
    if (count <= 0) {
        filter_destroy(filter);
        return count;
    }

    int on_heap = 0;
    size_t *indices = filter_ops_alloc_indices((size_t)count, &on_heap);
    if (!indices) {
        filter_destroy(filter);
        return -1;
    }

    /* Second pass: collect indices. */
    int collected = filter_ops_collect_matches(db, filter, indices, (size_t)count);
    filter_destroy(filter);

    if (collected < 0) {
        filter_ops_free_indices(indices, on_heap);
        return -1;
    }

    int updated = 0;
    for (int i = 0; i < collected; i++) {
        if (db_update_vector_metadata(db, indices[i],
                                          metadata_keys, metadata_values,
                                          metadata_count) == 0) {
            updated++;
        }
    }

    filter_ops_free_indices(indices, on_heap);
    return updated;
}

int db_count_by_filter(const GV_Database *db, const char *filter_expr)
{
    if (!db || !filter_expr) {
        return -1;
    }

    GV_Filter *filter = filter_parse(filter_expr);
    if (!filter) {
        return -1;
    }

    int count = filter_ops_collect_matches(db, filter, NULL, 0);
    filter_destroy(filter);
    return count;
}

int db_find_by_filter(const GV_Database *db, const char *filter_expr,
                          size_t *out_indices, size_t max_count)
{
    if (!db || !filter_expr || !out_indices || max_count == 0) {
        return -1;
    }

    GV_Filter *filter = filter_parse(filter_expr);
    if (!filter) {
        return -1;
    }

    int found = filter_ops_collect_matches(db, filter, out_indices, max_count);
    filter_destroy(filter);

    /* Cap the return value at max_count since the collect helper may have
     * counted more matches than the output buffer can hold. */
    if (found > 0 && (size_t)found > max_count) {
        return (int)max_count;
    }

    return found;
}
