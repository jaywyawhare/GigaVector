...
static int deserialize_links(const char *serialized, GV_MemoryLink **out, size_t *count) {
    ...
    while (*p && idx < link_count) {
        ...
        /* Store parsed link */
        links[idx].target_memory_id = strlen(target) > 0 ? gv_dup_cstr(target) : NULL;
        links[idx].link_type = (GV_MemoryLinkType)type;
        links[idx].strength = strength;
        links[idx].created_at = (time_t)created;
        links[idx].reason = strlen(reason) > 0 ? gv_dup_cstr(reason) : NULL;
        idx++;
...
GV_MemoryLayer *memory_layer_create(GV_Database *db, const GV_MemoryLayerConfig *config) {
    ...
    if (config != NULL) {
        ...
        layer->config = memory_layer_config_default();
    }
    ...
    if (layer->config.llm_config != NULL) {
        layer->llm = llm_create((GV_LLMConfig *)layer->config.llm_config);
    }
...
int memory_search(GV_MemoryLayer *layer, const float *query_embedding,
    ...
    if (indices != NULL) {
        ...
        importance_rerank(&config, contexts, importance_results, indices, count, 0.6);
...
char **memory_extract_from_conversation(GV_MemoryLayer *layer,
    ...
    if (layer->config.use_llm_extraction && layer->llm != NULL) {
        result = memory_extract_candidates_from_conversation_llm(
    ...
    if (result != 0 || actual_count == 0) {
        result = memory_extract_candidates_from_conversation(
    ...
    *embeddings = (float *)gv_alloc(actual_count * layer->db->dimension * sizeof(float));
    ...
    if (layer->config.enable_context_graph && layer->context_graph != NULL) {
        ...
        if (context_graph_extract((GV_ContextGraph *)layer->context_graph, conversation,
                                     NULL, NULL, NULL,
                                     &entities, &entity_count,
                                     &relationships, &relationship_count) == 0) {
            if (entity_count > 0) {
    ...
    for (size_t i = 0; i < actual_count; i++) {
        ...
        meta.source = conversation_id ? gv_dup_cstr(conversation_id) : NULL;
        meta.timestamp = time(NULL);
        meta.consolidated = 0;
...
char **memory_extract_from_text(GV_MemoryLayer *layer,
    ...
    int result = memory_extract_candidates_from_text(
    ...
    *embeddings = (float *)gv_alloc(actual_count * layer->db->dimension * sizeof(float));
    ...
    for (size_t i = 0; i < actual_count; i++) {
        ...
        meta.memory_type = candidates[i].memory_type;
        meta.importance_score = candidates[i].importance_score;
        meta.source = source ? gv_dup_cstr(source) : NULL;
        meta.timestamp = time(NULL);
        meta.consolidated = 0;
...
int memory_consolidate(GV_MemoryLayer *layer, double threshold, int strategy) {
    if (layer == NULL) {
        return -1;
    }
    ...
    int result = memory_find_similar(layer, actual_threshold, pairs, 1000, &pair_count);
    if (result != 0) {
        return -1;
    }
    ...
    for (size_t i = 0; i < pair_count; i++) {
        char *new_id = memory_consolidate_pair(layer, pairs[i].memory_id_1,
    ...
    memory_pairs_free(pairs, pair_count);
    return consolidated;
}
...
int memory_search_filtered(GV_MemoryLayer *layer, const float *query_embedding,
    ...
    int count;
    if (has_filter) {
        count = db_search_with_filter_expr(layer->db, query_embedding, k, search_results,
                                               distance_type, filter_expr);
    } else {
        count = db_search(layer->db, query_embedding, k, search_results, distance_type);
    }
    ...
    for (int i = 0; i < count && valid_count < k; i++) {
        ...
        if (content) {
            results[valid_count].content = gv_dup_cstr(content);
        }
        if (mem_id) {
...
int memory_search_advanced(GV_MemoryLayer *layer, const float *query_embedding,
    ...
    if (options != NULL) {
        ...
        opts = memory_search_options_default();
    }
    ...
    int count;
    if (has_filter) {
        count = db_search_with_filter_expr(layer->db, query_embedding, fetch_k,
                                               search_results, distance_type, filter_expr);
    } else {
        count = db_search(layer->db, query_embedding, fetch_k, search_results, distance_type);
    }
    ...
    GV_ImportanceConfig importance_config = importance_config_default();
    if (!layer->config.enable_temporal_weighting) {
        importance_config.weights.temporal_weight = 0.0;
    }

    importance_calculate_batch(&importance_config, contexts, importance_results, valid_count);
...
static int add_link_to_memory(GV_MemoryLayer *layer, const char *memory_id,
    ...
    for (size_t i = 0; i < result.metadata->link_count; i++) {
        if (result.metadata->links[i].target_memory_id &&
            ...
            ret = memory_update(layer, memory_id, NULL, result.metadata);
            memory_result_free(&result);
            return ret;
        }
    }
    ...
    new_link->target_memory_id = gv_dup_cstr(target_id);
    new_link->link_type = link_type;
    new_link->strength = strength;
    new_link->created_at = time(NULL);
    new_link->reason = reason ? gv_dup_cstr(reason) : NULL;
...
int memory_link_create(GV_MemoryLayer *layer,
    ...
    GV_MemoryLinkType reciprocal_type = memory_link_reciprocal(link_type);
    ...
    if (result.metadata != NULL && result.metadata->links != NULL) {
        ...
        for (size_t i = 0; i < link_count; i++) {
            links[i].target_memory_id = result.metadata->links[i].target_memory_id ?
                gv_dup_cstr(result.metadata->links[i].target_memory_id) : NULL;
            ...
            links[i].reason = result.metadata->links[i].reason ?
                gv_dup_cstr(result.metadata->links[i].reason) : NULL;
        }
    }
