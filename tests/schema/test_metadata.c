...
static int test_metadata_set_get(void) {
    ...
    GV_Vector *v = vector_create_from_data(3, v_data);
    ASSERT(v != NULL, "vector creation");
    
    ASSERT(vector_set_metadata(v, "key1", "value1") == 0, "set metadata");
    const char *val = vector_get_metadata(v, "key1");
...
static int test_metadata_multiple_keys(void) {
    ...
    ASSERT(vector_set_metadata(v, "key1", "value1") == 0, "set metadata 1");
    ASSERT(vector_set_metadata(v, "key2", "value2") == 0, "set metadata 2");
    ASSERT(vector_set_metadata(v, "key3", "value3") == 0, "set metadata 3");
    
    ASSERT(strcmp(vector_get_metadata(v, "key1"), "value1") == 0, "get key1");
    ASSERT(strcmp(vector_get_metadata(v, "key2"), "value2") == 0, "get key2");
    ASSERT(strcmp(vector_get_metadata(v, "key3"), "value3") == 0, "get key3");
...
static int test_metadata_update(void) {
    ...
    ASSERT(vector_set_metadata(v, "key1", "value1") == 0, "set metadata");
    ASSERT(vector_set_metadata(v, "key1", "value2") == 0, "update metadata");
...
static int test_metadata_remove(void) {
    ...
    ASSERT(vector_set_metadata(v, "key1", "value1") == 0, "set metadata");
    ASSERT(vector_remove_metadata(v, "key1") == 0, "remove metadata");
...
static int test_metadata_clear(void) {
    ...
    ASSERT(vector_get_metadata(v, "key1") == NULL, "cleared key1 should be NULL");
    ASSERT(vector_get_metadata(v, "key2") == NULL, "cleared key2 should be NULL");
...
static int test_metadata_nonexistent_key(void) {
    ...
    const char *val = vector_get_metadata(v, "nonexistent");
...
static int test_metadata_null_handling(void) {
    ...
    ASSERT(vector_set_metadata(NULL, "key", "value") < 0, "set metadata with NULL vector");
    ASSERT(vector_get_metadata(NULL, "key") == NULL, "get metadata with NULL vector");
    ASSERT(vector_remove_metadata(NULL, "key") < 0, "remove metadata with NULL vector");
...
static int test_metadata_in_database(void) {
    ...
    float v[2] = {1.0f, 2.0f};
    ASSERT(db_add_vector_with_metadata(db, v, 2, "tag", "test") == 0, "add vector with metadata");
