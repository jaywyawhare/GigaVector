...
static int test_codebook_create_destroy(void) {
    ...
    GV_Codebook *cb = codebook_create(4, 2, 8);
...
static int test_codebook_train(void) {
    GV_Codebook *cb = codebook_create(4, 2, 4); /* 16 centroids per subspace */
    ...
    ASSERT(codebook_train(cb, data, 64, 5) == 0, "train codebook with 64 vectors, 5 iters");
...
static int test_codebook_encode_decode(void) {
    ...
    ASSERT(codebook_train(cb, data, 32, 5) == 0, "train codebook");

    float vec[4] = {1.0f, 2.0f, 3.0f, 0.0f};
    uint8_t codes[2];
    ASSERT(codebook_encode(cb, vec, codes) == 0, "encode vector");

    ASSERT(codes[0] < cb->ksub, "code[0] in range");
    ASSERT(codes[1] < cb->ksub, "code[1] in range");

    float decoded[4];
    ASSERT(codebook_decode(cb, codes, decoded) == 0, "decode codes");
...
static int test_codebook_distance_adc(void) {
    ...
    uint8_t codes[2];
    codebook_encode(cb, vec, codes);

    float dist = codebook_distance_adc(cb, vec, codes);
    ASSERT(dist >= 0.0f, "ADC distance is non-negative");

    float far_query[4] = {100.0f, 100.0f, 100.0f, 100.0f};
    float dist_far = codebook_distance_adc(cb, far_query, codes);
...
static int test_codebook_copy(void) {
    ...
    codebook_train(cb, data, 32, 3);

    GV_Codebook *copy = codebook_copy(cb);
...
static int test_codebook_save_load(void) {
    ...
    ASSERT(codebook_save(cb, path) == 0, "save codebook to file");

    GV_Codebook *loaded = codebook_load(path);
...
static int test_codebook_save_load_fp(void) {
    ...
    codebook_train(cb, data, 64, 3);

    FILE *fout = fopen(path, "wb");
    ASSERT(fout != NULL, "open file for writing");
    ASSERT(codebook_save_fp(cb, fout) == 0, "save codebook via FILE*");
    fclose(fout);

    FILE *fin = fopen(path, "rb");
    ASSERT(fin != NULL, "open file for reading");
    GV_Codebook *loaded = codebook_load_fp(fin);
    fclose(fin);
