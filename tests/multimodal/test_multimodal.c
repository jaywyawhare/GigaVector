...
static int test_media_store_blob(void) {
    ...
    int rc = media_store_blob(store, 0, GV_MEDIA_IMAGE, blob_data, sizeof(blob_data),
                                  "test.png", "image/png");
    ASSERT(rc == 0, "storing blob should succeed");

    ASSERT(media_count(store) == 1, "should have 1 media entry");
...
static int test_media_retrieve(void) {
    ...
    int rc = media_store_blob(store, 10, GV_MEDIA_BLOB, original, sizeof(original),
                                  "data.bin", "application/octet-stream");
    ASSERT(rc == 0, "storing blob should succeed");

    unsigned char buffer[256];
    size_t actual_size = 0;
    rc = media_retrieve(store, 10, buffer, sizeof(buffer), &actual_size);
...
static int test_media_get_info(void) {
    ...
    int rc = media_store_blob(store, 5, GV_MEDIA_AUDIO, data, sizeof(data),
                                  "clip.wav", "audio/wav");
    ASSERT(rc == 0, "storing blob should succeed");

    GV_MediaEntry entry;
    memset(&entry, 0, sizeof(entry));
    rc = media_get_info(store, 5, &entry);
...
static int test_media_exists_and_delete(void) {
    ...
    media_store_blob(store, 20, GV_MEDIA_DOCUMENT, data, sizeof(data),
                        "doc.pdf", "application/pdf");

    ASSERT(media_exists(store, 20) == 1, "blob should exist at index 20");
    ASSERT(media_exists(store, 99) == 0, "blob should not exist at index 99");

    int rc = media_delete(store, 20);
    ASSERT(rc == 0, "deleting blob should succeed");
    ASSERT(media_exists(store, 20) == 0, "blob should not exist after deletion");
    ASSERT(media_count(store) == 0, "count should be 0 after deletion");
...
static int test_media_total_size(void) {
    ...
    ASSERT(media_total_size(store) == 0, "empty store should have 0 total size");

    const unsigned char data1[] = {0x01, 0x02, 0x03, 0x04, 0x05};
    const unsigned char data2[] = {0x10, 0x20, 0x30};

    media_store_blob(store, 0, GV_MEDIA_BLOB, data1, sizeof(data1), NULL, NULL);
    media_store_blob(store, 1, GV_MEDIA_BLOB, data2, sizeof(data2), NULL, NULL);

    size_t total = media_total_size(store);
...
static int test_media_get_path(void) {
    ...
    media_store_blob(store, 7, GV_MEDIA_IMAGE, data, sizeof(data),
                        "img.jpg", "image/jpeg");
