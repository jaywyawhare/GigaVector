...
static int test_quant_train_8bit(void) {
    ...
    GV_QuantCodebook *cb = quant_train(data, TRAIN_COUNT, DIM, &config);
    ASSERT(cb != NULL, "quant_train returned NULL for 8-bit");
...
static int test_quant_encode_decode_roundtrip(void) {
    ...
    float decoded[DIM];
    rc = quant_decode(cb, codes, DIM, decoded);
    ASSERT(rc == 0, "quant_decode failed");
...
static int test_quant_distance_asymmetric(void) {
    ...
    float dist = quant_distance(cb, data, DIM, codes);
...
static int test_quant_distance_symmetric(void) {
    ...
    rc = quant_encode(cb, data, DIM, codes_b);
    ASSERT(rc == 0, "encode b failed");

    float dist = quant_distance_qq(cb, codes_a, codes_b, DIM);
...
static int test_quant_memory_ratio(void) {
    ...
    float ratio = quant_memory_ratio(cb, DIM);
