...
static int test_named_vectors_add_field(void) {
    ...
    ASSERT(named_vectors_add_field(store, &cfg1) == 0, "add field 'title'");
    ASSERT(named_vectors_add_field(store, &cfg2) == 0, "add field 'content'");
    ASSERT(named_vectors_field_count(store) == 2, "field count is 2");

    GV_VectorFieldConfig out;
    ASSERT(named_vectors_get_field(store, "title", &out) == 0, "get field 'title'");
...
static int test_named_vectors_remove_field(void) {
    ...
    named_vectors_add_field(store, &cfg);
    ASSERT(named_vectors_field_count(store) == 1, "field count is 1");

    ASSERT(named_vectors_remove_field(store, "temp") == 0, "remove field 'temp'");
    ASSERT(named_vectors_field_count(store) == 0, "field count is 0 after removal");

    ASSERT(named_vectors_remove_field(store, "nonexistent") == -1,
...
static int test_named_vectors_insert_and_get(void) {
    ...
    named_vectors_add_field(store, &cfg_title);
    named_vectors_add_field(store, &cfg_body);
    ...
    ASSERT(named_vectors_insert(store, 0, vectors, 2) == 0, "insert point 0");
    ASSERT(named_vectors_count(store) == 1, "count is 1 after insert");

    const float *retrieved = named_vectors_get(store, 0, "title");
    ASSERT(retrieved != NULL, "get title vector for point 0");
    ASSERT(retrieved[0] == 1.0f, "title[0] == 1.0");

    retrieved = named_vectors_get(store, 0, "body");
...
static int test_named_vectors_update(void) {
    ...
    named_vectors_insert(store, 0, &vec, 1);

    float data_v2[4] = {5.0f, 6.0f, 7.0f, 8.0f};
    GV_NamedVector updated = { .field_name = "embed", .data = data_v2, .dimension = 4 };
    ASSERT(named_vectors_update(store, 0, &updated, 1) == 0, "update point 0");

    const float *retrieved = named_vectors_get(store, 0, "embed");
...
static int test_named_vectors_delete(void) {
    ...
    named_vectors_insert(store, 0, &v1, 1);
    named_vectors_insert(store, 1, &v2, 1);
    ASSERT(named_vectors_count(store) == 2, "count is 2");

    ASSERT(named_vectors_delete(store, 0) == 0, "delete point 0");

    const float *gone = named_vectors_get(store, 0, "data");
...
static int test_named_vectors_search(void) {
    ...
    named_vectors_insert(store, 0, &v0, 1);
    named_vectors_insert(store, 1, &v1, 1);
    named_vectors_insert(store, 2, &v2, 1);

    float query[4] = {1.0f, 0.0f, 0.0f, 0.0f};
    GV_NamedSearchResult results[3];
    int n = named_vectors_search(store, "embed", query, 2, results);
...
static int test_named_vectors_save_load(void) {
    ...
    named_vectors_insert(store, 0, &v, 1);

    ASSERT(named_vectors_save(store, path) == 0, "save named vectors");
    named_vectors_destroy(store);

    GV_NamedVectorStore *loaded = named_vectors_load(path);
    ASSERT(loaded != NULL, "load named vectors");
    ASSERT(named_vectors_field_count(loaded) == 1, "loaded field count is 1");
    ASSERT(named_vectors_count(loaded) == 1, "loaded point count is 1");

    const float *r = named_vectors_get(loaded, 0, "vec");
    ASSERT(r != NULL, "get vector from loaded store");
    ASSERT(r[0] == 1.0f && r[3] == 4.0f, "loaded vector data correct");

    named_vectors_destroy(loaded);
    remove(path);
    return 0;
}
