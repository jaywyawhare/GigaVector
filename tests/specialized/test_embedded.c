...
static int test_open_close(void) {
    ...
    GV_EmbeddedDB *db = embedded_open(&cfg);
    ASSERT(db != NULL, "embedded db open");
    ASSERT(embedded_count(db) == 0, "empty db should have count 0");
...
static int test_add_count(void) {
    ...
    int idx1 = embedded_add(db, v1);
    ASSERT(idx1 >= 0, "add v1 should succeed");
    int idx2 = embedded_add(db, v2);
    ASSERT(idx2 >= 0, "add v2 should succeed");
    ASSERT(embedded_count(db) == 2, "count should be 2");
...
static int test_add_with_id_and_get(void) {
    ...
    int rc = embedded_add_with_id(db, 5, vec);
    ASSERT(rc == 0, "add with id=5 should succeed");

    float out[DIM] = {0};
    rc = embedded_get(db, 5, out);
...
static int test_search(void) {
    ...
    int found = embedded_search(db, query, 3, 0, results);
...
static int test_delete_compact(void) {
    ...
    embedded_add(db, v);
    embedded_add(db, v);
    ASSERT(embedded_count(db) == 3, "count should be 3 before delete");

    int rc = embedded_delete(db, 1);
    ASSERT(rc == 0, "delete index 1 should succeed");
    ASSERT(embedded_count(db) == 2, "count should be 2 after delete");

    rc = embedded_compact(db);
...
static int test_memory_usage(void) {
    ...
    size_t mem_before = embedded_memory_usage(db);
    ...
    size_t mem_after = embedded_memory_usage(db);
