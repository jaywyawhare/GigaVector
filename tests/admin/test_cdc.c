...
static int test_create_destroy(void) {
    GV_CDCStream *stream = cdc_create(NULL);
    ASSERT(stream != NULL, "cdc_create(NULL) should succeed");
    cdc_destroy(stream);

    GV_CDCConfig cfg;
    cdc_config_init(&cfg);
    stream = cdc_create(&cfg);
    ASSERT(stream != NULL, "cdc_create with config should succeed");
    cdc_destroy(stream);
...
static int test_publish_and_poll(void) {
    ...
    GV_CDCCursor cursor = cdc_get_cursor(stream);
    ...
    ASSERT(cdc_publish(stream, &event) == 0, "publish insert");

    GV_CDCEvent polled[4];
    memset(polled, 0, sizeof(polled));
    int n = cdc_poll(stream, &cursor, polled, 4);
...
static int test_subscribe_and_callback(void) {
    ...
    int sub_id = cdc_subscribe(stream, GV_CDC_INSERT | GV_CDC_DELETE,
                                  test_callback, NULL);
    ...
    evt.vector_data = vec;
    evt.dimension = 1;
    ASSERT(cdc_publish(stream, &evt) == 0, "publish insert");
    ...
    ASSERT(cdc_unsubscribe(stream, sub_id) == 0, "unsubscribe");

    evt.type = GV_CDC_DELETE;
    evt.vector_data = NULL;
    ASSERT(cdc_publish(stream, &evt) == 0, "publish delete");
...
static int test_cursor_from_sequence(void) {
    GV_CDCCursor c = cdc_cursor_from_sequence(100);
    ASSERT(c.sequence_number == 100, "cursor sequence_number should be 100");

    GV_CDCCursor c0 = cdc_cursor_from_sequence(0);
...
static int test_pending_count(void) {
    ...
    ASSERT(cdc_pending_count(stream, &cursor) == 0, "no pending initially");
    ...
    for (int i = 0; i < 3; i++) {
        ...
        ASSERT(cdc_publish(stream, &evt) == 0, "publish");
    }

    size_t pending = cdc_pending_count(stream, &cursor);
...
static int test_multiple_event_types(void) {
    ...
    ASSERT(cdc_publish(stream, &evt) == 0, "publish update");
    ...
    int n = cdc_poll(stream, &cursor, polled, 8);
