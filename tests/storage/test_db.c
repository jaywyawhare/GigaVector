...
static int test_open_close(void) {
    GV_Database *db = db_open(NULL, 3, GV_INDEX_TYPE_KDTREE);
...
static int test_add_and_search(void) {
    ...
    ASSERT(db_add_vector(db, v1, 2) == 0, "add vector");
...
static int test_save_load_and_wal(void) {
    ...
    float v[2] = {0.1f, 0.2f};
    ASSERT(db_add_vector_with_metadata(db, v, 2, "tag", "a") == 0, "add with metadata");
    ASSERT(db_save(db, NULL) == 0, "save");
    db_close(db);

    GV_Database *db2 = db_open(path, 2, GV_INDEX_TYPE_KDTREE);
...
static int test_all_index_types(void) {
    ...
    for (int t = 0; t < 3; t++) {
        GV_Database *db = db_open(NULL, 8, types[t]);
        if (db == NULL) {
            continue;
        }
        ...
        if (types[t] == GV_INDEX_TYPE_IVFPQ) {
            ...
            ASSERT(db_ivfpq_train(db, train_data, 256, 8) == 0, "train IVFPQ");
        }
...
static int test_rich_metadata(void) {
    ...
    ASSERT(db_add_vector_with_rich_metadata(db, v, 2, keys, values, 3) == 0, "add with rich metadata");
...
static int test_batch_operations(void) {
    ...
    ASSERT(db_add_vectors(db, vectors, 10, 3) == 0, "batch add vectors");
...
static int test_delete_vector(void) {
    ...
    int delete_result = db_delete_vector_by_index(db, 1);
    if (delete_result != 0) {
        db_close(db);
        return 0;
    }
...
static int test_update_vector(void) {
    ...
    int update_result = db_update_vector(db, 0, new_v, 2);
    if (update_result != 0) {
        db_close(db);
        return 0;
    }
...
static int test_update_metadata(void) {
    ...
    float v[2] = {1.0f, 2.0f};
    ASSERT(db_add_vector_with_metadata(db, v, 2, "tag", "old") == 0, "add with metadata");
    
    const char *keys[] = {"tag", "owner"};
    const char *values[] = {"new", "user"};
    int update_result = db_update_vector_metadata(db, 0, keys, values, 2);
    db_close(db);
    if (update_result != 0) {
        return 0;
    }
...
static int test_error_handling(void) {
    ...
    float v[3] = {1.0f, 2.0f, 3.0f};
    ASSERT(db_add_vector(db, v, 3) < 0, "wrong dimension should fail");
    
    float v2[2] = {1.0f, 2.0f};
    ASSERT(db_add_vector(NULL, v2, 2) < 0, "NULL db should fail");
    
    ASSERT(db_search(NULL, v2, 1, NULL, GV_DISTANCE_EUCLIDEAN) < 0, "NULL db search should fail");
    
    ASSERT(db_delete_vector_by_index(db, 999) < 0, "delete non-existent index should fail");
...
static int test_wal_operations(void) {
    ...
    float v[2] = {1.0f, 2.0f};
    ASSERT(db_add_vector_with_metadata(db, v, 2, "tag", "test") == 0, "add with metadata");
...
static int test_exact_search_threshold(void) {
    ...
    db_set_exact_search_threshold(db, 10);
    db_set_force_exact_search(db, 1);
