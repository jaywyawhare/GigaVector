...
static int test_open_from_memory(void) {
    ...
    GV_Database *db1 = db_open(path, 3, GV_INDEX_TYPE_KDTREE);
    ASSERT(db1 != NULL, "create db");
    
    float v[3] = {1.0f, 2.0f, 3.0f};
    ASSERT(db_add_vector(db1, v, 3) == 0, "add vector");
    ASSERT(db_save(db1, NULL) == 0, "save");
...
static int test_open_mmap(void) {
    ...
    GV_Database *db2 = db_open_mmap(path, 3, GV_INDEX_TYPE_KDTREE);
    if (db2 != NULL) {
        float q[3] = {1.0f, 2.0f, 3.0f};
        GV_SearchResult res[1];
        int n = db_search(db2, q, 1, res, GV_DISTANCE_EUCLIDEAN);
...
static int test_index_suggest(void) {
    ...
    ASSERT(idx == GV_INDEX_TYPE_IVFDISK, "very large high-D over budget suggests IVFDisk (db_open-buildable; DISKANN is not)");
...
static int test_resource_limits(void) {
    ...
    ASSERT(db_set_resource_limits(db, &limits) == 0, "set resource limits");
    
    GV_ResourceLimits retrieved = {0};
    db_get_resource_limits(db, &retrieved);
...
static int test_compaction(void) {
    ...
    db_set_compaction_interval(db, 60);
    db_set_wal_compaction_threshold(db, 1024 * 1024);
    db_set_deleted_ratio_threshold(db, 0.1);
    
    float v[2] = {1.0f, 2.0f};
    ASSERT(db_add_vector(db, v, 2) == 0, "add vector");
    
    ASSERT(db_compact(db) == 0, "manual compact");
...
static int test_background_compaction(void) {
    ...
    if (db_start_background_compaction(db) == 0) {
        float v[2] = {1.0f, 2.0f};
        ASSERT(db_add_vector(db, v, 2) == 0, "add vector");
...
static int test_record_latency_recall(void) {
    ...
    db_record_latency(db, 1000, 1);
    db_record_latency(db, 500, 0);
    db_record_recall(db, 0.95);
...
static int test_range_search_filtered(void) {
    ...
    ASSERT(db_add_vector_with_metadata(db, v1, 2, "tag", "a") == 0, "add vector 1");
    ASSERT(db_add_vector_with_metadata(db, v2, 2, "tag", "b") == 0, "add vector 2");
    ASSERT(db_add_vector_with_metadata(db, v3, 2, "tag", "a") == 0, "add vector 3");
...
static int test_add_vectors_with_metadata(void) {
    ...
    ASSERT(db_add_vectors_with_metadata(db, vectors, keys, values, 3, 3) == 0, "batch add with metadata");
