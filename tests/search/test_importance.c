...
int test_informativeness_empty(void) {
    double score = importance_informativeness(NULL, 0);
    ASSERT_RANGE(score, 0.0, 0.0, "null input");

    score = importance_informativeness("", 0);
...
int test_informativeness_simple(void) {
    ...
    double score = importance_informativeness(simple, strlen(simple));
...
int test_informativeness_complex(void) {
    ...
    double score = importance_informativeness(complex, strlen(complex));
    ASSERT_RANGE(score, 0.3, 0.9, "complex sentence");

    const char *simple = "I like cats.";
    double simple_score = importance_informativeness(simple, strlen(simple));
...
int test_specificity_numbers(void) {
    ...
    double score_with = importance_specificity(with_numbers, strlen(with_numbers));
    double score_without = importance_specificity(without_numbers, strlen(without_numbers));
...
int test_specificity_proper_nouns(void) {
    ...
    double score_with = importance_specificity(with_names, strlen(with_names));
    double score_without = importance_specificity(without_names, strlen(without_names));
...
int test_specificity_vague_words(void) {
    ...
    double score_vague = importance_specificity(vague, strlen(vague));
    double score_specific = importance_specificity(specific, strlen(specific));
...
int test_salience_emotional(void) {
    ...
    double score_emotional = importance_salience(emotional, strlen(emotional));
    double score_neutral = importance_salience(neutral, strlen(neutral));
...
int test_salience_sentence_emphasis(void) {
    ...
    double score_emphasized = importance_salience(emphasized, strlen(emphasized));
    double score_plain = importance_salience(plain, strlen(plain));
...
int test_salience_important_markers(void) {
    ...
    double score_important = importance_salience(important, strlen(important));
    double score_normal = importance_salience(normal, strlen(normal));
...
int test_entity_density(void) {
    ...
    double score_high = importance_entity_density(high_entity, strlen(high_entity));
    double score_low = importance_entity_density(low_entity, strlen(low_entity));
...
int test_content_score_combined(void) {
    ...
    double score_high = importance_score_content(high_quality, strlen(high_quality));
    double score_low = importance_score_content(low_quality, strlen(low_quality));
...
int test_temporal_decay_immediate(void) {
    double decay = importance_temporal_decay(NULL, 0);
...
int test_temporal_decay_one_hour(void) {
    double decay = importance_temporal_decay(NULL, 3600);  /* 1 hour in seconds */
...
int test_temporal_decay_one_day(void) {
    double decay = importance_temporal_decay(NULL, 86400);  /* 24 hours */
...
int test_temporal_decay_one_week(void) {
    double decay = importance_temporal_decay(NULL, 604800);  /* 7 days */
...
int test_temporal_decay_one_month(void) {
    double decay = importance_temporal_decay(NULL, 2592000);  /* 30 days */
...
int test_temporal_decay_minimum(void) {
    double decay = importance_temporal_decay(NULL, 31536000);  /* 1 year */
...
int test_temporal_recency_boost(void) {
    ...
    double decay_12hours = importance_temporal_decay(NULL, 43200);  /* 12 hours - within window */
    double decay_48hours = importance_temporal_decay(NULL, 172800); /* 48 hours - outside window */
...
int test_temporal_custom_config(void) {
    ...
    double decay = importance_temporal_decay(&config, 86400);  /* 1 day */
...
int test_access_history_init(void) {
    ...
    int result = access_history_init(&history, 16);
...
int test_access_record(void) {
    ...
    int result = importance_record_access(&history, now, 0.8, 0);
...
int test_access_multiple_records(void) {
    ...
    time_t now = time(NULL);
    importance_record_access(&history, now - 3600, 0.6, 0);
    importance_record_access(&history, now - 1800, 0.8, 0);
    importance_record_access(&history, now, 1.0, 0);
...
int test_access_score_no_history(void) {
    double score = importance_access_score(NULL, NULL, time(NULL));
...
int test_access_score_with_history(void) {
    ...
    importance_record_access(&history, now - 172800, 0.7, 0);  /* 2 days ago */
    importance_record_access(&history, now - 86400, 0.8, 0);   /* 1 day ago */
    importance_record_access(&history, now, 0.9, 0);           /* now */

    double score = importance_access_score(NULL, &history, now);
...
int test_access_score_frequent_better(void) {
    ...
    access_history_init(&history1, 16);
    access_history_init(&history2, 16);

    time_t now = time(NULL);

    importance_record_access(&history1, now, 0.8, 0);

    for (int i = 0; i < 10; i++) {
        importance_record_access(&history2, now - i * 3600, 0.8, 0);
    }

    double score1 = importance_access_score(NULL, &history1, now);
    double score2 = importance_access_score(NULL, &history2, now);

    ASSERT_GT(score2, score1, "frequent access scores higher");

    access_history_free(&history1);
    access_history_free(&history2);
    TEST_PASS();
    return 0;
}
...
int test_access_history_serialization(void) {
    ...
    time_t now = time(NULL);
    importance_record_access(&history, now - 3600, 0.6, 0);
    importance_record_access(&history, now, 0.8, 1);

    char *json = access_history_serialize(&history);
...
int test_importance_calculate_basic(void) {
    ...
    int ret = importance_calculate(NULL, &ctx, &result);
...
int test_importance_calculate_with_access(void) {
    ...
    time_t now = time(NULL);
    importance_record_access(&history, now - 86400, 0.8, 0);
    importance_record_access(&history, now, 0.9, 0);
...
int test_importance_old_vs_new(void) {
    ...
    importance_calculate(NULL, &ctx_new, &result_new);
    importance_calculate(NULL, &ctx_old, &result_old);
...
int test_importance_batch(void) {
    ...
    int scored = importance_calculate_batch(NULL, contexts, results, 3);
...
int test_importance_rerank(void) {
    ...
    int ret = importance_rerank(NULL, contexts, results, indices, 4, 0.5);
...
int test_config_custom_weights(void) {
    ...
    GV_ImportanceResult result;
    importance_calculate(&config, &ctx, &result);
